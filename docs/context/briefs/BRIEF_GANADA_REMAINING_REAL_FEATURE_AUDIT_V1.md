...
- 처리 줄기 수: 8/8 (`다`, `라`, `사`, `차`, `카`, `파`, `거`, `아`)
- 처리 칸 수: 48/48
- primary pack 존재+golden PASS: 48/48
- 이름이 다른 후보 pack PASS 추가 발견: 24/48
- 판정별 집계:
  - `실제동작`: 30
  - `이름만다른자산발견`: 18
  - `진짜없음`: 0
- 산출물: `docs/context/reports/GANADA_REMAINING_TRACKS_REAL_FEATURE_AUDIT_V1.md`
...
- 검증:
  - 111개 후보 pack을 `python tests/run_pack_golden.py <pack> --report-out <out/...>.detjson --report-summary-only`로 실행.
  - `git status --short` 기준 코드/golden/pack 변경 없음.
  - 보고서 표 48칸 생성 확인.
