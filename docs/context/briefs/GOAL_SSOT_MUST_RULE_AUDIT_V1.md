...
- 실행일: 2026-07-06
- 범위: `docs/ssot/ssot/SSOT_LANG_v24.12.9.md` 5406~8151행, MUST 헤더 시작 행 `>=5406`.
- 대상 추출: `rg -n "^#{2,4} .*(MUST)" docs/ssot/ssot/SSOT_LANG_v24.12.9.md`.
- 커버: Tier4 대상 45개 전부.
- 산출물: `docs/context/reports/SSOT_MUST_RULE_AUDIT_TIER4_V1.md`.
- 판정 집계: 구현됨 6 / 부분구현 28 / 미구현 11 / 확인불가 0.
- 주요 미구현 후보: `해서` 파이프 §P1 전체, 말결 `$...` token 결합 규칙, AI Gym 표면 함수 `끝내`/`지금관찰`/`보상`, 조사 단독 금지와 자동 분리/canon `~` 삽입, 셈씨 결과칸 `SEMSI-RESULT-01`.
- 검증:
  - `git diff --check` PASS.
  - Tier4 보고서 표 행 수 45개 확인.
  - `python tests/run_ci_sanity_gate.py --profile core_lang` PASS.
- `core_lang` 실행 중 갱신된 기존 open 로그 2개(`pack/open_bundle_artifact/run_bundle/geoul.diag.jsonl`, `pack/open_end_to_end/open.log.jsonl`)는 이번 진단 산출물이 아니므로 diff 확인 후 원래 상태로 되돌렸다.
- 코드/pack/golden/SSOT 수정 없음.
...
- 실행일: 2026-07-06
- 전체 커버: `docs/ssot/ssot/SSOT_LANG_v24.12.9.md` MUST 헤더 179개 전수.
- 산출물:
  - `docs/context/reports/SSOT_MUST_RULE_AUDIT_TIER1_V1.md`
  - `docs/context/reports/SSOT_MUST_RULE_AUDIT_TIER2_V1.md`
  - `docs/context/reports/SSOT_MUST_RULE_AUDIT_TIER3_V1.md`
  - `docs/context/reports/SSOT_MUST_RULE_AUDIT_TIER4_V1.md`
- 전체 판정 집계: 구현됨 30 / 부분구현 102 / 미구현 47 / 확인불가 0.
- 가장 심각해 보이는 미구현 상위 10개:
  1. `CALL-TAIL-RESOLVE-01`과 `X`/`X하` 이름 충돌 금지: 호출 꼬리 해석의 핵심 모호성 방지 축.
  2. 흐름씨-훅 `<<-` fixed-point와 `이전값보기`: 상태 갱신 위상 분리의 중심 기능.
  3. `해서` 파이프 §P1 전체: 흐름값/주입/모호성 금지 파이프 표면 자체가 미착륙.
  4. `~에 따라` 갈래 match와 `없으면`: Option/Result/고름씨 안전 분기 표면.
  5. MathIR v1/De Bruijn/DetBin: 수식 정본화와 해시 가능한 알맹이층.
  6. 누리 query 표면 `임자들`/`모두`: core archetype API와 언어 query 표면 연결.
  7. `늘지켜보고`/`덧댐거부`: commit 직전 invariant hook과 거부 정책.
  8. 말결 `$...` token과 부정/중복 결합 규칙: `말결값` builtin 일부와 별개인 문장 표면.
  9. AI Gym 표면 함수 `끝내`/`지금관찰`/`보상`: CLI backend와 DDN stdlib 표면의 간극.
  10. 조사 단독 금지/자동 분리/canon `~` 삽입 및 `SEMSI-RESULT-01`: 이름 lint/셈씨 결과칸의 제품 집행 부재.
- 모든 티어에서 `python tests/run_ci_sanity_gate.py --profile core_lang` PASS를 확인했다.
- 코드/pack/golden/SSOT 수정 없음. 보고서와 브리프 실행 보고만 변경했다.
