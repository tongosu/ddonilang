    ...
    fn hash_component_data(hasher: &mut Hasher, data: &ComponentData) {
        match data {
            ...
            ComponentData::Tuple(items) | ComponentData::List(items) => {
                hasher.update(if matches!(data, ComponentData::Tuple(_)) {
                    b"t"
                } else {
                    b"l"
                });
...
pub enum ReplayDifference {
    ...
    /// 틱 범위 불일치
    TickRangeMismatch { expected_end: u64, actual_end: u64 },
}
...
impl ReplayVerifier {
    ...
    pub fn verify_replay(
        ...
        // 2. 틱별 상태 해시 검증
        for ((orig_tick, orig_hash), (replay_tick, replay_hash)) in original_trace
            .tick_hashes
            .iter()
            .zip(&replay_trace.tick_hashes)
        {
