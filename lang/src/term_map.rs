...
pub const FATAL_TERMS: [TermEntry; 12] = [
    ...
    TermEntry { code: "TERM-FATAL-007", input: "에러", canonical: "고장" },
    TermEntry { code: "TERM-FATAL-008", input: "랜덤", canonical: "주사위" },
    TermEntry { code: "TERM-FATAL-009", input: "디버그", canonical: "거울" },
    TermEntry { code: "TERM-FATAL-010", input: "상태", canonical: "누리" },
    TermEntry { code: "TERM-FATAL-011", input: "로그", canonical: "진단말" },
...
pub fn find_fatal_term(term: &str) -> Option<TermEntry> {
    FATAL_TERMS.iter().copied().find(|entry| entry.input == term)
}

pub fn find_legacy_term(term: &str) -> Option<TermEntry> {
    LEGACY_TERMS.iter().copied().find(|entry| entry.input == term)
}
