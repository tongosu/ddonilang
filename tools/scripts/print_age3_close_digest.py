...
def main() -> int:
    parser = argparse.ArgumentParser(description="Print digest from ddn.seamgrim.age3_close_report.v1")
    parser.add_argument("report", help="path to age3 close report detjson")
    parser.add_argument("--top", type=int, default=5, help="max failure digest lines")
    ...
    if payload is None:
        print(f"[age3-close] report missing_or_invalid: {path}")
        return 0
    ...
    print(f"[age3-close] overall_ok={int(overall_ok)} criteria={total} failed={failed} report={path}")
