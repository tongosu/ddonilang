...
def main() -> int:
    parser = argparse.ArgumentParser(description="Print digest from ddn.age4_close_report.v1")
    parser.add_argument("report", help="path to age4 close report detjson")
    ...
    if payload is None:
        print(f"[age4-close] report missing_or_invalid: {path}")
        return 0
    ...
    print(f"[age4-close] overall_ok={int(overall_ok)} criteria={total} failed={failed} report={path}")
