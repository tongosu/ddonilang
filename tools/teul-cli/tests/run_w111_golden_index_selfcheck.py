...
SCRIPT_TAG = "w111-golden-index-selfcheck"
PROGRESS_ENV_KEY = "DDN_W111_GOLDEN_INDEX_SELFCHECK_PROGRESS_JSON"
CASE_DIR_RE = re.compile(r"^W111_(G\d{2})_")
CASE_CODE_RE = re.compile(r"^G\d{2}$")
ALLOWED_CLASSIFICATIONS = {"noop", "runtime_error", "parse_error", "success", "hash_invariant"}
...
def validate_case_expectations(
    *,
    ...
    trace_hash = expect.get("trace_hash")

    if classification in {"success", "hash_invariant"}:
        if exit_code != 0:
            raise ValueError(f"{case_dir}: classification={classification} must have expect.exit=0")
        return

    if classification == "noop":
        if exit_code != 0:
            raise ValueError(f"{case_dir}: classification=noop must have expect.exit=0")
        if stderr_contains:
            raise ValueError(f"{case_dir}: classification=noop must have empty expect.stderr_contains")
        return

    if classification == "runtime_error":
        if exit_code != 1:
            raise ValueError(f"{case_dir}: classification=runtime_error must have expect.exit=1")
        if not stderr_contains:
            raise ValueError(f"{case_dir}: classification=runtime_error must have non-empty expect.stderr_contains")
        if not any("E_RUNTIME_" in line for line in stderr_contains):
            raise ValueError(f"{case_dir}: classification=runtime_error must include E_RUNTIME_* in expect.stderr_contains")
        ...
        if state_hash != "ABSENT" or trace_hash != "ABSENT":
            raise ValueError(f"{case_dir}: classification=runtime_error must have state_hash/trace_hash = ABSENT")
        return

    if classification == "parse_error":
        if exit_code != 1:
            raise ValueError(f"{case_dir}: classification=parse_error must have expect.exit=1")
        if not stderr_contains:
            raise ValueError(f"{case_dir}: classification=parse_error must have non-empty expect.stderr_contains")
        if not any("E_PARSE_" in line for line in stderr_contains):
            raise ValueError(f"{case_dir}: classification=parse_error must include E_PARSE_* in expect.stderr_contains")
        if diag_code and diag_code not in stderr_contains:
            raise ValueError(f"{case_dir}: missing diag_code={diag_code} in expect.stderr_contains")
        if state_hash != "ABSENT" or trace_hash != "ABSENT":
            raise ValueError(f"{case_dir}: classification=parse_error must have state_hash/trace_hash = ABSENT")
        return

    raise ValueError(f"{case_dir}: unknown classification={classification}")


def main() -> int:
    progress_path = os.environ.get(PROGRESS_ENV_KEY, "")
    root = Path(__file__).resolve().parent
    case_root = root / "golden" / "W111"
    ...
    if index.get("walk") != 111:
        return fail_probe("index walk must be 111", probe="validate_index_meta", last_probe="read_index_json")
    ...
    for idx, item in enumerate(cases):
        ...
        if not case_dir.startswith(f"W111_{code}_"):
            return fail_probe(
        ...
        if spec.get("walk") != 111:
            return fail_probe(
                f"{case_dir}: spec walk must be 111",
                probe="validate_case_specs",
                last_probe="validate_index_meta",
            )
        if spec.get("command") != "run":
            return fail_probe(
                f"{case_dir}: spec command must be run",
                probe="validate_case_specs",
                last_probe="validate_index_meta",
            )
    ...
    expected_codes = {f"G{i:02d}" for i in range(1, 36)}
    ...
    print(
        ...
        f"runtime={sum(1 for c in cases if c.get('classification') == 'runtime_error')} "
        f"parse={sum(1 for c in cases if c.get('classification') == 'parse_error')} "
        f"noop={sum(1 for c in cases if c.get('classification') == 'noop')}"
    )
    return 0
