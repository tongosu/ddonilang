...
`이름 밝히기 { ... }.` 즉시 실행형 proof 표면이 `세움{}` / `살피기`, `중 딱 하나가` 양화, `해찾기`, proof artifact 봉인을 함께 유지하는 AGE1 회귀 팩.
...
- 실행:
  - `python tests/run_pack_golden.py age1_immediate_proof_solver_search_solve_smoke_v1`
  - `python tests/run_pack_golden.py --update age1_immediate_proof_solver_search_solve_smoke_v1`
