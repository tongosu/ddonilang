...
def fail(msg: str) -> int:
    print(f"[ci-aggregate-age5-child-summary-gate0-surface-family-transport-contract-selftest] fail: {msg}")
    return 1
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(
        prefix="ci_aggregate_age5_child_summary_gate0_surface_family_transport_contract_selftest_"
    ) as tmp:
        work = Path(tmp)
        ...
        expected_defaults = {
            ...
            "age5_full_real_gate0_surface_family_transport_contract_selftest_current_probe": "-",
            "age5_full_real_gate0_surface_family_transport_contract_selftest_last_completed_probe": "-",
            "age5_full_real_gate0_surface_family_transport_contract_selftest_progress_present": "0",
        }
        for key, expected_value in expected_defaults.items():
            if summary.get(key, "") != expected_value:
                return fail(f"default mismatch {key}: {summary.get(key)} != {expected_value}")

    print("[ci-aggregate-age5-child-summary-gate0-surface-family-transport-contract-selftest] ok")
    return 0
