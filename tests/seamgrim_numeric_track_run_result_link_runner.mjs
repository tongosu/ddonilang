...
const OK = "seamgrim_numeric_track_run_result_link: ok";
...
async function main() {
  ...
  try {
    ...
    await page.waitForSelector(`.lesson-card[data-lesson-id="${TARGET_ID}"] .card-launch-btn[data-launch-profile="student"]`);
    await page.click(`.lesson-card[data-lesson-id="${TARGET_ID}"] .card-launch-btn[data-launch-profile="student"]`);
    await waitVisible(page, "#screen-run");
    await page.waitForFunction(() => {
      const link = window.__SEAMGRIM_NUMERIC_TRACK_RUN_RESULT_LINK__;
      return link?.schema === "seamgrim.numeric_track_run_result_link.v1"
        && String(link.state_hash ?? "").startsWith("blake3:");
    }, null, { timeout: 30000 });

    const state = await page.evaluate(() => {
      const chip = document.querySelector("[data-run-result-numeric-link]");
      return {
        chipText: chip?.textContent?.trim() ?? "",
        chipHidden: chip?.classList?.contains("hidden") ?? true,
        chipValue: chip?.dataset?.value ?? "",
        link: window.__SEAMGRIM_NUMERIC_TRACK_RUN_RESULT_LINK__ ?? null,
        text: window.__SEAMGRIM_NUMERIC_TRACK_RUN_RESULT_LINK_TEXT__ ?? "",
        prefs: JSON.parse(window.localStorage.getItem("seamgrim.ui.run_prefs.v1") || "{}"),
      };
    });
    ...
    assert(state.link?.preset_focus === "근/구간", `link focus mismatch: ${state.link?.preset_focus}`);
    assert(state.link?.run_kind, "link run kind missing");
    assert(Number(state.link?.channels ?? -1) >= 0, "link channels invalid");
    assert(String(state.link?.state_hash ?? "").startsWith("blake3:"), "link state hash missing");
    assert(String(state.text).includes("schema\tseamgrim.numeric_track_run_result_link.v1"), "link text schema missing");
    assert(String(state.text).includes(`lesson_id\t${TARGET_ID}`), "link text lesson missing");
    assert(String(state.text).includes("preset_focus\t근/구간"), "link text focus missing");
    assert(state.prefs?.lessons?.[TARGET_ID]?.numericTrackRunResultLink?.schema === "seamgrim.numeric_track_run_result_link.v1", "localStorage link missing");
