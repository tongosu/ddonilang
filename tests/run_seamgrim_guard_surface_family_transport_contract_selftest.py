...
SCRIPT_TAG = "seamgrim-guard-surface-family-transport-contract-selftest"
SELF_SCRIPT_PATH = "tests/run_seamgrim_guard_surface_family_transport_contract_selftest.py"
PROGRESS_ENV_KEY = "DDN_SEAMGRIM_GUARD_SURFACE_FAMILY_TRANSPORT_CONTRACT_SELFTEST_PROGRESS_JSON"
CHECKS = (
    ("family_contract", "tests/run_seamgrim_guard_surface_family_contract_selftest.py"),
    ("ddn_exec_server_surface", "tests/run_seamgrim_ddn_exec_server_surface_contract_selftest.py"),
    ("runtime_fallback_surface", "tests/run_seamgrim_runtime_fallback_surface_contract_selftest.py"),
)
CHECKS_TEXT = ",".join(name for name, _ in CHECKS)
OPTIONAL_CHECKS = (
    ("full_gate_surface_optional", "tests/run_seamgrim_full_gate_surface_contract_selftest.py"),
)
FAMILY_CONTRACT_NAME = "family_contract"
FAMILY_CONTRACT_COVERED_CHECKS = {
    "ddn_exec_server_surface",
    "runtime_fallback_surface",
}
...
def write_progress_snapshot(
    ...
    *,
    ...
    payload = {
        "schema": "ddn.ci.seamgrim_guard_surface_family_transport_contract_selftest.progress.v1",
...
def main() -> int:
    ...
    for name, script in OPTIONAL_CHECKS:
        if is_script_cached(script):
            print(f"[{SCRIPT_TAG}] cache-hit check={name} script={script}")
            continue
        proc = subprocess.run(
        ...
        if proc.returncode != 0:
            ...
            stderr = (proc.stderr or "").strip()
            detail = stderr or stdout or "-"
            optional_failures.append(f"{name}:{detail}")
            continue
        mark_script_ok(script)
    ...
    if optional_failures:
        print(f"[{SCRIPT_TAG}] warn optional_failures={';'.join(optional_failures)}")
