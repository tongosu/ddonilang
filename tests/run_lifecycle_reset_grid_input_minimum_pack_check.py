...
PACK_SCHEMA = "ddn.lifecycle_reset_grid_input_minimum.pack.contract.v1"
RUN_CASES = {
...
SMOKE_CASE = ("c05_state_view_boundary", "smoke_with_view_boundary.v1.json")


def fail(code: str, msg: str) -> int:
    print(f"[lifecycle-reset-grid-input-minimum-pack-check] fail code={code} msg={msg}", file=sys.stderr)
    return 1
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Lifecycle/reset/grid/input minimum pack checker")
    parser.add_argument("--pack", default="pack/lifecycle_reset_grid_input_minimum_v1")
    ...
    if missing:
        return fail("E_LIFECYCLE_MIN_FILE_MISSING", ",".join(missing))

    try:
        contract = load_json(pack / "contract.detjson")
        golden = load_jsonl(pack / "golden.jsonl")
    except ValueError as exc:
        return fail("E_LIFECYCLE_MIN_INVALID", str(exc))

    if not isinstance(contract, dict):
        return fail("E_LIFECYCLE_MIN_CONTRACT_TYPE", "contract root must be object")
    if str(contract.get("schema", "")).strip() != PACK_SCHEMA:
        return fail("E_LIFECYCLE_MIN_SCHEMA", f"schema={contract.get('schema')}")
    if str(contract.get("evidence_tier", "")).strip() != "runner_fill":
        return fail("E_LIFECYCLE_MIN_TIER", f"tier={contract.get('evidence_tier')}")
    if str(contract.get("closure_claim", "")).strip() != "no":
        return fail("E_LIFECYCLE_MIN_CLOSURE", f"closure={contract.get('closure_claim')}")
    for rel in contract.get("evidence_targets", []):
        if not Path(rel).exists():
            return fail("E_LIFECYCLE_MIN_TARGET", rel)

    golden_index = {str(row.get("id", "")).strip(): row for row in golden}
    if set(golden_index) != set(RUN_CASES) | {SMOKE_CASE[0]}:
        return fail("E_LIFECYCLE_MIN_GOLDEN_SET", f"golden={sorted(golden_index)}")

    for case_id, expected_cmd in RUN_CASES.items():
        expected = load_json(pack / "cases" / case_id / "expected.json")
        if not isinstance(expected, dict):
            return fail("E_LIFECYCLE_MIN_EXPECTED", case_id)
        golden_row = golden_index[case_id]
        if golden_row.get("cmd") != expected_cmd:
            return fail("E_LIFECYCLE_MIN_CMD", case_id)
        if golden_row.get("stdout") != expected.get("stdout"):
            return fail("E_LIFECYCLE_MIN_STDOUT", case_id)
        if int(golden_row.get("exit_code", -1)) != 0:
            return fail("E_LIFECYCLE_MIN_EXIT", case_id)

    smoke_case_id, smoke_target = SMOKE_CASE
    smoke_expected = load_json(pack / "cases" / smoke_case_id / "expected.json")
    smoke_row = golden_index[smoke_case_id]
    if str(smoke_row.get("smoke_golden", "")).strip() != smoke_target:
        return fail("E_LIFECYCLE_MIN_SMOKE_TARGET", smoke_case_id)
    if str(smoke_expected.get("target", "")).strip() != smoke_target:
        return fail("E_LIFECYCLE_MIN_SMOKE_EXPECTED", smoke_case_id)
    if int(smoke_row.get("exit_code", -1)) != 0:
        return fail("E_LIFECYCLE_MIN_SMOKE_EXIT", smoke_case_id)

    print("[lifecycle-reset-grid-input-minimum-pack-check] ok")
