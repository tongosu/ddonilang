...
BUNDLE = (
    ...
    / "2"
    / "ddonirang_vol2_project_currentline_bundle_20260501_r2.zip"
)
...
STATIC_REPORT = (
    ...
    / "context"
    / "reports"
    / "DDONIRANG_VOL2_BUNDLE_EXECUTION_CLASSIFICATION_20260502.md"
)
...
DDN_HINTS = ("채비", "보여주기", "보임", "차림", "거르기", "변환", "합치기", "정렬")
CATEGORIES = {"ready", "ssot_surface_drift", "runtime_missing_surface", "doc_example_error"}
CAUSES = {
    "collection_type_surface",
    "range_josa_spacing",
    "collection_pipeline_sugar",
    "multi_param_lambda",
}


def fail(message: str) -> int:
    print(f"[ddonirang-vol2-bundle-cli-wasm-parity] fail: {message}")
    return 1


def build_base() -> Path:
    base = PRIMARY_BUILD if PRIMARY_BUILD.exists() else FALLBACK_BUILD
    out = base / "ddonirang_vol2_bundle_cli_wasm_parity"
        ...
        cases.append(
            {
                ...
                "expected_cli_wasm_row_keys": resource_keys(body),
                "source": source_name,
            }
        )
    contract = {
        "schema": "ddn.seamgrim.wasm_cli_runtime_parity.pack.contract.v1",
        "pack_id": "ddonirang_vol2_project_currentline_bundle_20260501_r2",
...
def feature_flags(body: str) -> dict[str, bool]:
    return {
        "collection_type_surface": bool(
            re.search(r"\([^)]+\)\s*차림|\([^)]+\)차림", body)
        ),
        "range_josa_spacing": bool(re.search(r"\d+\.\.\d+에\s+대해", body)),
        "collection_pipeline_sugar": any(token in body for token in ("~을", "~로", "~부터")),
        "multi_param_lambda": bool(re.search(r"\{\s*\([^)]+,[^)]+\)\s*\|", body)),
        "boim": "보임" in body,
    }
...
def classify_case(runner_case: dict) -> str:
    ...
    if "parse" in failures.lower() or "acceptance mismatch" in failures.lower():
...
def write_classification_artifact(
    ...
    payload = {
        "schema": "ddn.ddonirang_vol2.bundle_execution_classification.v1",
        "bundle": str(BUNDLE),
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="run_", dir=base) as tmp:
        ...
        for idx in range(1, 19):
            token = f"vol2_{idx:03d}"
            if token not in static_text:
                return fail(f"static report missing {token}")

    print("ddonirang vol2 bundle cli/wasm runtime parity ok")
    return 0
