...
CHECK_SCRIPT = Path(__file__).resolve().parent / "run_seamgrim_ci_gate_pack_evidence_tier_step_check.py"
CHECK_REPORT_SCHEMA = "ddn.seamgrim_ci_gate_pack_evidence_tier_step_check.v1"
EXPECTED_FILE_COUNT = 3
MISSING_TOKEN_REL_PATH = "tests/run_seamgrim_ci_gate.py"
MISSING_TOKEN_VALUE = '"pack_evidence_tier_selftest"'


def fail(msg: str) -> int:
    print(f"[seamgrim-ci-gate-pack-evidence-tier-step-check-selftest] fail {msg}", file=sys.stderr)
    return 1
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Selftest for pack evidence tier step contract checker")
    parser.add_argument(
        "--verify-report",
        help="optional real report path emitted by run_seamgrim_ci_gate_pack_evidence_tier_step_check.py",
    )
    return parser.parse_args()


def load_check_module() -> ModuleType:
    spec = importlib.util.spec_from_file_location("seamgrim_pack_evidence_tier_step_check_mod", CHECK_SCRIPT)
...
def main() -> int:
    args = parse_args()
    with tempfile.TemporaryDirectory(prefix="seamgrim_pack_evidence_tier_step_selftest_") as tmp:
        root = Path(tmp)
    ...
    print("[seamgrim-ci-gate-pack-evidence-tier-step-check-selftest] ok")
    return 0
