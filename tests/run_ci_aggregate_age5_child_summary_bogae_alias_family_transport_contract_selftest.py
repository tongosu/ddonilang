...
def fail(msg: str) -> int:
    print(f"[ci-aggregate-age5-child-summary-bogae-alias-family-transport-contract-selftest] fail: {msg}")
    return 1
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(
        prefix="ci_aggregate_age5_child_summary_bogae_alias_family_transport_contract_selftest_"
    ) as tmp:
        work = Path(tmp)
        ...
        write_json(
            age5_report,
            {
                ...
                "age5_full_real_bogae_alias_family_transport_contract_selftest_checks_text": (
                    ...
                    "gate_result,gate_outputs_consistency,gate_summary_line,final_line_emitter,report_index"
                ),
        ...
        expected_defaults = {
            ...
            "age5_full_real_bogae_alias_family_transport_contract_selftest_current_probe": "-",
            "age5_full_real_bogae_alias_family_transport_contract_selftest_last_completed_probe": "-",
            "age5_full_real_bogae_alias_family_transport_contract_selftest_progress_present": "0",
        }
        for key, expected_value in expected_defaults.items():
            if summary.get(key, "") != expected_value:
                return fail(f"default mismatch {key}: {summary.get(key)} != {expected_value}")

    print("[ci-aggregate-age5-child-summary-bogae-alias-family-transport-contract-selftest] ok")
    return 0
