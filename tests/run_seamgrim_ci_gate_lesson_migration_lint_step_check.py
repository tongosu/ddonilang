...
FILE_TOKEN_REQUIREMENTS: dict[str, tuple[str, ...]] = {
    "tests/run_seamgrim_ci_gate.py": (
        ...
        "tests/run_seamgrim_ci_gate_lesson_migration_lint_step_check.py",
        '"seamgrim_ci_gate_lesson_migration_lint_step_check_selftest"',
        "tests/run_seamgrim_ci_gate_lesson_migration_lint_step_check_selftest.py",
    ),
    "tests/_seamgrim_ci_diag_lib.py": (
        'elif name == "lesson_migration_lint":',
        "lesson_migration_priority_nonzero",
        "lesson_migration_lint_detail",
        'elif name == "seamgrim_ci_gate_lesson_migration_lint_step_check":',
        "lesson_migration_step_check_failed",
        "lesson_migration_step_check_token_missing",
        'elif name == "seamgrim_ci_gate_lesson_migration_lint_step_check_selftest":',
        "lesson_migration_step_selftest_failed",
    ),
...
def build_report(*, root: Path, missing: list[str]) -> dict[str, object]:
    return {
        "schema": "ddn.seamgrim_ci_gate_lesson_migration_lint_step_check.v1",
        "status": "pass" if not missing else "fail",
        "ok": not missing,
        "code": "OK" if not missing else "E_SEAMGRIM_LESSON_MIGRATION_LINT_STEP_CONTRACT_MISSING",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check lesson_migration_lint wiring across seamgrim ci gate flow")
    parser.add_argument(
        "--repo-root",
    ...
    if missing:
        print("seamgrim ci gate lesson migration lint step check failed:")
        for token in missing[:12]:
            print(f" - missing token: {token}")
        return 1

    print("seamgrim ci gate lesson migration lint step check ok")
