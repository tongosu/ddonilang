...
def safe_stage_token(text: str) -> str:
    return re.sub(r"[^A-Za-z0-9._,-]+", "_", str(text).strip())
...
def write_progress_snapshot(
    ...
    *,
    ...
    payload = {
        ...
        "last_completed_pack": last_completed_pack,
        "current_case": current_case,
        "last_completed_case": last_completed_case,
        "total_elapsed_ms": int(total_elapsed_ms),
    }
    tmp_path = out.with_name(f"{out.name}.{time.time_ns()}.tmp")
...
def run_subprocess_with_stack_retry_progress(
    cmd: list[str],
    cwd: Path,
    *,
    timeout: float | None = None,
    progress_hook=None,
    stage_prefix: str = "run_command",
) -> subprocess.CompletedProcess[str]:
    env = dict(os.environ)
    env.setdefault("RUST_MIN_STACK", str(64 * 1024 * 1024))
    env.setdefault("CARGO_TARGET_DIR", str((cwd / "build" / "cargo-target-pack-golden").resolve()))
    ...
    for attempt in range(STACK_OVERFLOW_RETRY_MAX):
        ...
        if completed.returncode == 0:
            return completed
        if not is_stack_overflow_like(completed):
            return completed
        if attempt < STACK_OVERFLOW_RETRY_MAX - 1:
            time.sleep(0.2 * (attempt + 1))
    assert last is not None
...
def make_temp_report_path(root: Path, stem: str, case_idx: int) -> Path:
    temp_dir = root / "build" / "tmp"
    temp_dir.mkdir(parents=True, exist_ok=True)
    suffix = time.time_ns()
    return temp_dir / f"{stem}_{case_idx}_{suffix}.detjson"
...
def iter_packs(root: Path, names: list[str], use_all: bool) -> list[Path]:
    ...
    default_packs = [
        ...
        pack_root / "gate0_contract_abort_statehash_v1",
        ...
        pack_root / "bogae_canvas_key_precedence_v1",
        pack_root / "bogae_canvas_ssot_key_precedence_v1",
        pack_root / "bogae_drawlist_listkey_v1",
        pack_root / "bogae_drawlist_trait_alias_v1",
        pack_root / "bogae_shape_trait_ssot_alias_v1",
        pack_root / "bdl2_subpixel_aa_v1",
        pack_root / "age1_charim_index",
        pack_root / "age1_immediate_proof_smoke_v1",
        pack_root / "age2_dice_seed_smoke_v1",
        pack_root / "age2_open_input_record_replay_v1",
        pack_root / "age3_beat_reserve_smoke_v1",
        pack_root / "age4_proof_detjson_smoke_v1",
        pack_root / "age4_quantifier_surface_smoke_v1",
        pack_root / "age4_case_completeness_smoke_v1",
        pack_root / "age4_proof_solver_translation_smoke_v1",
        pack_root / "age4_proof_solver_open_replay_v1",
        pack_root / "age4_proof_solver_search_replay_v1",
        pack_root / "age4_proof_solver_deny_failure_v1",
        pack_root / "age4_proof_solver_replay_missing_failure_v1",
        pack_root / "age4_proof_solver_replay_tamper_failure_v1",
        pack_root / "age4_proof_solver_replay_parse_failure_v1",
        pack_root / "age4_proof_clock_replay_missing_failure_v1",
        pack_root / "age4_proof_file_read_replay_missing_failure_v1",
        pack_root / "age4_proof_input_replay_missing_failure_v1",
        pack_root / "age4_proof_clock_replay_tamper_failure_v1",
        pack_root / "age4_proof_clock_replay_parse_failure_v1",
        pack_root / "age4_proof_file_read_replay_tamper_failure_v1",
        pack_root / "age4_proof_file_read_replay_parse_failure_v1",
        pack_root / "age4_proof_input_replay_tamper_failure_v1",
        pack_root / "age4_proof_input_replay_parse_failure_v1",
        ...
        pack_root / "lang_consistency_v1",
        pack_root / "lang_teulcli_parser_parity_v1",
        pack_root / "lang_surface_family_v1",
        pack_root / "module_system_smoke_v1",
        ...
        pack_root / "seamgrim_graph_v0_basics",
        pack_root / "seamgrim_graph_autorender_v1",
...
def load_cases(pack_dir: Path) -> list[dict]:
    ...
    for idx, line in enumerate(lines, 1):
        ...
        has_core_expectation = (
            ...
            or "expected_state_transition_report" in data
        )
        has_graph_expectation = "fixture" in data and "expected_graph" in data
        has_graph_export_expectation = "expected_graph" in data and "fixture" not in data
        ...
        if (
            ...
            and not has_graph_export_expectation
            ...
            raise ValueError(
                f"{golden_path} line {idx}: missing stdout/stdout_path, bogae_hash, expected_error_code, expected_warning_code, smoke_golden, expected_state_transition_report, expected_graph, fixture/expected_graph, dotbogi_case, overlay_compare_case, overlay_session_case, or guideblock_case"
            )
        ...
        if "cwd" in data and not isinstance(data["cwd"], str):
            raise ValueError(f"{golden_path} line {idx}: cwd must be a string")
        ...
        if "state_transition_report_out" in data and not isinstance(data["state_transition_report_out"], str):
            raise ValueError(f"{golden_path} line {idx}: state_transition_report_out must be a string")
        if "expected_state_transition_report" in data and not isinstance(data["expected_state_transition_report"], str):
            raise ValueError(f"{golden_path} line {idx}: expected_state_transition_report must be a string")
        ...
        if ("state_transition_report_out" in data) ^ ("expected_state_transition_report" in data):
            raise ValueError(
                f"{golden_path} line {idx}: state_transition_report_out and expected_state_transition_report must be provided together"
            )
        validate_case_mode_exclusivity(golden_path, idx, data)
        validate_graph_export_case_contract(golden_path, idx, data)
...
def validate_case_mode_exclusivity(golden_path: Path, line_no: int, data: dict) -> None:
    ...
    has_graph = ("fixture" in data) and ("expected_graph" in data)
...
def validate_graph_export_case_contract(golden_path: Path, line_no: int, data: dict) -> None:
    if "expected_graph" not in data or "fixture" in data:
        return
    exit_code = int(data.get("exit_code", 0))
    if exit_code != 0:
        raise ValueError(
            f"{golden_path} line {line_no}: expected_graph(export) requires exit_code=0"
        )
    if data.get("expected_error_code") is not None:
        raise ValueError(
            f"{golden_path} line {line_no}: expected_graph(export) must not set expected_error_code"
        )
    cmd = data.get("cmd")
    if cmd is None:
        return
    if not isinstance(cmd, list) or not cmd:
        raise ValueError(
            f"{golden_path} line {line_no}: expected_graph(export) cmd must be a non-empty list"
        )
    argv = [str(item) for item in cmd]
    if argv[0] != "run":
        raise ValueError(
            f"{golden_path} line {line_no}: expected_graph(export) requires run cmd when cmd is provided"
        )
...
def validate_run_case_contract(golden_path: Path, line_no: int, data: dict) -> None:
    ...
    if ("state_transition_report_out" in data) ^ ("expected_state_transition_report" in data):
        raise ValueError(
            f"{golden_path} line {line_no}: state_transition_report_out and expected_state_transition_report must be provided together"
        )
...
def map_contract_from_policy(run_policy: dict) -> str:
    ...
    if not det:
        return ""
    normalized = det.replace("_", "").replace("-", "")
    if normalized == "DSTRICT":
        return "D-STRICT"
    if normalized in ("DSEALED", "DFAST"):
        return "D-SEALED"
    if normalized in ("DAPPROX", "DULTRA"):
        return "D-APPROX"
    if det:
        return "D-APPROX"
    return ""
...
def sha256_json_obj_prefixed(value: dict) -> str:
...
def validate_bundle_manifest_provenance(
    exec_cwd: Path,
    command_args: list[str],
) -> tuple[bool, list[str], list[str]] | None:
    if len(command_args) < 3 or command_args[0] != "bundle" or command_args[1] != "parity":
        return None
    bundle_in = (exec_cwd / command_args[2]).resolve()
    manifest_path = bundle_in / "manifest.detjson"
    if not manifest_path.exists():
        return (
            False,
            [f"bundle_manifest={manifest_path}"],
            ["bundle_manifest missing"],
        )
    manifest = load_json_doc(manifest_path)
    if not isinstance(manifest, dict):
        raise ValueError(f"{manifest_path}: manifest root must be object")
    provenance = manifest.get("source_provenance")
    if not isinstance(provenance, dict):
        raise ValueError(f"{manifest_path}: manifest.source_provenance must be object")
    if provenance.get("schema") != "seulgi.bundle_source_provenance.v1":
        raise ValueError(f"{manifest_path}: manifest.source_provenance.schema mismatch")
    if provenance.get("source_kind") != "bundle_parity_inputs.v1":
        raise ValueError(f"{manifest_path}: manifest.source_provenance.source_kind mismatch")

    bundle_root = bundle_in.parent
    expected_files = {
        "model_file": bundle_in / "model_mlp_v1.detjson",
        "weights_file": bundle_in / "weights_v1.detbin",
        "eval_report_file": bundle_in / "eval_report.detjson",
        "eval_config_file": bundle_root / "eval_config.json",
        "artifact_file": bundle_root / "artifact.detjson",
    }
    for key, path in expected_files.items():
        expected_name = path.name
        actual_name = provenance.get(key)
    ...
    expected_hashes = {
        "model_hash": sha256_file_prefixed(expected_files["model_file"]),
        "weights_hash": sha256_file_prefixed(expected_files["weights_file"]),
        "eval_report_hash": sha256_file_prefixed(expected_files["eval_report_file"]),
        "eval_config_hash": sha256_file_prefixed(expected_files["eval_config_file"]),
        "artifact_hash": sha256_file_prefixed(expected_files["artifact_file"]),
    }
    ...
    expected_source_hash = sha256_json_obj_prefixed(provenance)
    actual_source_hash = manifest.get("source_hash")
    if actual_source_hash != expected_source_hash:
        return (
            False,
            [f"source_hash={expected_source_hash}"],
            [f"source_hash={actual_source_hash}"],
        )
    for manifest_key in ("artifact_hash", "eval_report_hash"):
        expected_value = expected_hashes[manifest_key]
        actual_value = manifest.get(manifest_key)
        if actual_value != expected_value:
            return (
                False,
                [f"{manifest_key}={expected_value}"],
                [f"{manifest_key}={actual_value}"],
            )
    return True, [], []
...
def run_dotbogi_case(
    ...
    if not isinstance(source_hash, str) or not source_hash.strip():
        raise ValueError(f"{doc_path}: report.source_hash must be non-empty string")
    expected_source_hash = sha256_file_prefixed(doc_path)
    ...
    if not isinstance(source_provenance, dict):
        raise ValueError(f"{doc_path}: report.source_provenance must be object")
    if source_provenance.get("schema") != "ddn.dotbogi.case.source_provenance.v1":
        raise ValueError(f"{doc_path}: report.source_provenance.schema mismatch")
    if source_provenance.get("source_kind") != "dotbogi_case.v1":
        raise ValueError(f"{doc_path}: report.source_provenance.source_kind mismatch")
    expected_input_case_file = str(doc_path).replace("\\", "/")
    if source_provenance.get("input_case_file") != expected_input_case_file:
        return (
            False,
            [f"input_case_file={expected_input_case_file}"],
            [f"input_case_file={source_provenance.get('input_case_file')}"],
            "\n".join(stderr_lines),
            pre_artifacts,
        )
    if source_provenance.get("input_case_hash") != expected_source_hash:
        return (
            False,
            [f"input_case_hash={expected_source_hash}"],
            [f"input_case_hash={source_provenance.get('input_case_hash')}"],
            "\n".join(stderr_lines),
            pre_artifacts,
        )
...
def run_overlay_compare_case(
    ...
    report_path = make_temp_report_path(root, "overlay_compare_case_report", case_idx)
    ...
    expected_group_id = str(row.get("expected_group_id", "") or "") if isinstance(row, dict) else ""
    actual_group_id = str(actual.get("group_id", "") or "") if isinstance(actual, dict) else ""
    actual_baseline_group_id = str(row.get("actual_baseline_group_id", "") or "") if isinstance(row, dict) else ""
    actual_variant_group_id = str(row.get("actual_variant_group_id", "") or "") if isinstance(row, dict) else ""
    ...
    if expected_group_id:
        expected_lines.append(f"group_id={expected_group_id}")
        got_lines.append(f"group_id={actual_group_id}")
    if actual_baseline_group_id:
        got_lines.append(f"baseline_group_id={actual_baseline_group_id}")
    if actual_variant_group_id:
        got_lines.append(f"variant_group_id={actual_variant_group_id}")
...
def run_overlay_session_case(
    ...
    report_path = make_temp_report_path(root, "overlay_session_case_report", case_idx)
    cleanup_temp_file(report_path)
    cmd = [
        "node",
        "--no-warnings",
    ...
    expected_baseline_group_id = str(row.get("expected_baseline_group_id", "") or "") if isinstance(row, dict) else ""
    expected_variant_group_id = str(row.get("expected_variant_group_id", "") or "") if isinstance(row, dict) else ""
    actual_baseline_group_id = str(row.get("actual_baseline_group_id", "") or "") if isinstance(row, dict) else ""
    actual_variant_group_id = str(row.get("actual_variant_group_id", "") or "") if isinstance(row, dict) else ""
    ...
    if expected_baseline_group_id:
        expected_lines.append(f"baseline_group_id={expected_baseline_group_id}")
    if expected_variant_group_id:
        expected_lines.append(f"variant_group_id={expected_variant_group_id}")
    if actual_baseline_group_id:
        got_lines.append(f"baseline_group_id={actual_baseline_group_id}")
    if actual_variant_group_id:
        got_lines.append(f"variant_group_id={actual_variant_group_id}")
...
def run_guideblock_case(
    ...
    result = run_subprocess_with_stack_retry_progress(
        cmd,
        cwd=root,
        progress_hook=progress_hook,
        stage_prefix="run_command",
    )
    if progress_hook is not None:
        progress_hook("collect_output")
    stderr_lines = [
        normalize_stderr_line(line, root)
        for line in result.stderr.splitlines()
        if line.strip()
...
def run_graph_autorender_case(
    ...
    if not expected_path.exists():
        if allow_missing_expected:
            ...
            raise ValueError(
                f"{pack_dir}/golden.jsonl case {case_idx}: expected_graph not found: {expected_rel}"
            )
    else:
...
def run_graph_export_case(
    ...
    graph_exporter = root / "solutions" / "seamgrim_ui_mvp" / "tools" / "export_graph.py"
    output_path = root / "build" / "tmp" / f"graph_export_case_{case_idx}_{time.time_ns()}.json"
    output_path.parent.mkdir(parents=True, exist_ok=True)
    cmd = [sys.executable, str(graph_exporter), str(run_path), str(output_path)]
    result = run_subprocess_with_stack_retry(
        cmd,
        cwd=root,
    )
    ...
    stdout_lines = [line for line in result.stdout.splitlines() if line.strip()]
    if result.returncode != 0:
        detail = stderr_lines or stdout_lines or ["graph export failed"]
        cleanup_temp_file(output_path)
        return (
            False,
            ["graph export exit_code=0"],
            [f"graph export exit_code={result.returncode}"],
            "\n".join(detail),
            {
                "stdout_lines": stdout_lines,
                "stderr_lines": stderr_lines,
    ...
    if not output_path.exists():
        return (
            False,
            ["graph export output exists"],
            ["graph export output missing"],
            "\n".join(stderr_lines + stdout_lines),
            {
                "stdout_lines": stdout_lines,
                "stderr_lines": stderr_lines,
    ...
    try:
        actual_obj = json.loads(output_path.read_text(encoding="utf-8"))
    finally:
        cleanup_temp_file(output_path)
...
def resolve_graph_export_input_path(
    command_args: list[str],
    exec_cwd: Path,
    run_path: Path,
) -> Path:
    if len(command_args) >= 2 and str(command_args[0]) == "run":
        second = str(command_args[1]).strip()
        if second and not second.startswith("-"):
            candidate = Path(second)
            if not candidate.is_absolute():
                return (exec_cwd / candidate).resolve()
...
def run_case(
    ...
    if isinstance(guideblock_case, str) and guideblock_case.strip():
        return run_guideblock_case(root, pack_dir, case_idx, case, progress_hook=progress_hook)
    if "fixture" in case and "expected_graph" in case:
        return run_graph_autorender_case(
            root,
            pack_dir,
            case_idx,
            case,
    ...
    state_transition_report_out = case.get("state_transition_report_out")
    expected_state_transition_report_path = case.get("expected_state_transition_report")
    ...
    expected_graph_path = case.get("expected_graph")
    temp_path = None

    if input_text is not None:
        if progress_hook is not None:
            progress_hook("prepare_input")
    ...
    if isinstance(case_cwd, str) and case_cwd.strip():
        exec_cwd = (pack_dir / case_cwd).resolve()
    ...
    actual_state_transition_report_path: Path | None = None
    if state_transition_report_out:
        actual_state_transition_report_path = exec_cwd / str(state_transition_report_out)
        actual_state_transition_report_path.unlink(missing_ok=True)
    result = run_subprocess_with_stack_retry_progress(
        cmd,
        cwd=exec_cwd,
        progress_hook=progress_hook,
    ...
    actual_state_transition_report: str | None = None
    if meta_out:
        meta_out_path = pack_dir / meta_out
        if meta_out_path.exists():
            actual_meta = meta_out_path.read_text(encoding="utf-8").strip()
            meta_out_path.unlink(missing_ok=True)
    if actual_state_transition_report_path is not None and actual_state_transition_report_path.exists():
        actual_state_transition_report = actual_state_transition_report_path.read_text(encoding="utf-8").strip()
        actual_state_transition_report_path.unlink(missing_ok=True)
    if progress_hook is not None:
        progress_hook("collect_artifacts")
    pre_artifacts = {
        "stdout_lines": [line for line in stdout_raw if line.strip()],
        "stderr_lines": stderr_lines,
        "exit_code": result.returncode,
        "actual_meta": actual_meta,
        "actual_state_transition_report": actual_state_transition_report,
    }
    ...
    if expected_error_code is not None:
        ...
        if meta_out and expected_meta_path:
            expected_meta = load_expected_text(
                pack_dir,
                expected_meta_path,
                allow_missing_expected,
            )
            ...
            if expected_meta is not None and actual_meta != expected_meta:
                return (
                    False,
        ...
        if state_transition_report_out and expected_state_transition_report_path:
            expected_report_file = pack_dir / expected_state_transition_report_path
            if allow_missing_expected and not expected_report_file.exists():
                expected_report = None
            else:
                expected_report = expected_report_file.read_text(encoding="utf-8").strip()
            if actual_state_transition_report is None:
                return (
                    False,
                    ["state_transition_report missing"],
                    ["state_transition_report missing"],
                    "\n".join(stderr_lines),
                    pre_artifacts,
                )
            if expected_report is not None and actual_state_transition_report != expected_report:
                return (
                    False,
                    expected_report.splitlines(),
                    actual_state_transition_report.splitlines(),
                    "\n".join(stderr_lines),
                    pre_artifacts,
                )
        if (
    ...
    if stdout_path:
        expected = load_expected_stdout_lines(pack_dir, stdout_path, allow_missing_expected)
    else:
    ...
    bundle_manifest_check = validate_bundle_manifest_provenance(exec_cwd, command_args)
    if bundle_manifest_check is not None:
        ok, expected_lines, actual_lines = bundle_manifest_check
        if not ok:
            return False, expected_lines, actual_lines, "\n".join(stderr_lines), artifacts
    if state_transition_report_out and expected_state_transition_report_path:
        expected_report_file = pack_dir / expected_state_transition_report_path
    ...
    if isinstance(expected_graph_path, str) and expected_graph_path.strip() and "fixture" not in case:
        graph_input_path = resolve_graph_export_input_path(command_args, exec_cwd, run_path)
        graph_temp_input_path: Path | None = None
        if not graph_input_path.exists() and input_text is not None:
            graph_temp_input_path = pack_dir / f".tmp_graph_case_{case_idx}.ddn"
            graph_temp_input_path.write_text(input_text, encoding="utf-8", newline="\n")
            graph_input_path = graph_temp_input_path
        try:
            graph_ok, graph_expected, graph_actual, graph_stderr, graph_artifacts = run_graph_export_case(
                root,
                pack_dir,
                case_idx,
                graph_input_path,
            ...
            artifacts["graph_expected_path"] = graph_artifacts.get("graph_expected_path")
            artifacts["graph_actual_json"] = graph_artifacts.get("graph_actual_json")
            if not graph_ok:
                merged_stderr = "\n".join(
                    line
                    for line in [graph_stderr, "\n".join(stderr_lines)]
                    if isinstance(line, str) and line.strip()
                )
                return False, graph_expected, graph_actual, merged_stderr, artifacts
        finally:
            if graph_temp_input_path is not None:
                cleanup_temp_file(graph_temp_input_path)
...
def load_expected_stdout_lines(
    pack_dir: Path,
    stdout_path: str,
    allow_missing_expected: bool,
) -> list[str]:
    expected_file = pack_dir / stdout_path
    if allow_missing_expected and not expected_file.exists():
        return []
    return expected_file.read_text(encoding="utf-8").splitlines()
...
def load_expected_text(
    ...
    expected_file = pack_dir / expected_path
    if allow_missing_expected and not expected_file.exists():
        return None
...
def collect_pack_expected_paths(pack_dir: Path, cases: list[dict]) -> list[Path]:
    expected_paths: set[Path] = {pack_dir / "golden.jsonl"}
    for case in cases:
        dotbogi_case = case.get("dotbogi_case")
        if isinstance(dotbogi_case, str) and dotbogi_case.strip():
            expected_paths.add((pack_dir / dotbogi_case).resolve())
        overlay_compare_case = case.get("overlay_compare_case")
        if isinstance(overlay_compare_case, str) and overlay_compare_case.strip():
            expected_paths.add((pack_dir / overlay_compare_case).resolve())
        overlay_session_case = case.get("overlay_session_case")
        if isinstance(overlay_session_case, str) and overlay_session_case.strip():
            expected_paths.add((pack_dir / overlay_session_case).resolve())
        guideblock_case = case.get("guideblock_case")
        if isinstance(guideblock_case, str) and guideblock_case.strip():
            expected_paths.add((pack_dir / guideblock_case).resolve())
        stdout_path = case.get("stdout_path")
        if isinstance(stdout_path, str) and stdout_path.strip():
            expected_paths.add((pack_dir / stdout_path).resolve())
        expected_meta = case.get("expected_meta")
        if isinstance(expected_meta, str) and expected_meta.strip():
            expected_paths.add((pack_dir / expected_meta).resolve())
        expected_state_transition_report = case.get("expected_state_transition_report")
        if isinstance(expected_state_transition_report, str) and expected_state_transition_report.strip():
            expected_paths.add((pack_dir / expected_state_transition_report).resolve())
        smoke_golden = case.get("smoke_golden")
        if isinstance(smoke_golden, str) and smoke_golden.strip():
            expected_paths.add((pack_dir / smoke_golden).resolve())
        expected_graph = case.get("expected_graph")
        if isinstance(expected_graph, str) and expected_graph.strip():
            expected_paths.add((pack_dir / expected_graph).resolve())
        expected_dotbogi_output = case.get("expected_dotbogi_output")
        if isinstance(expected_dotbogi_output, str) and expected_dotbogi_output.strip():
            expected_paths.add((pack_dir / expected_dotbogi_output).resolve())
        expected_after_state = case.get("expected_after_state")
        if isinstance(expected_after_state, str) and expected_after_state.strip():
            expected_paths.add((pack_dir / expected_after_state).resolve())
    return sorted(path for path in expected_paths if path.exists())


def write_pack_metadata(pack_dir: Path, cases: list[dict], run_log_lines: list[str]) -> None:
    expected_paths = collect_pack_expected_paths(pack_dir, cases)
    sha_lines = []
    for path in expected_paths:
        rel = path.relative_to(pack_dir).as_posix()
        sha_lines.append(f"sha256:{sha256_file(path)}  {rel}")
...
def write_case_updates(
    ...
    cmd_override = case.get("cmd")
    if cmd_override is not None:
        is_run_cmd = isinstance(cmd_override, list) and len(cmd_override) > 0 and cmd_override[0] == "run"
    else:
        is_run_cmd = True
    if is_run_cmd:
        stdout_lines = [
    ...
    expected_state_transition_report_path = case.get("expected_state_transition_report")
    actual_state_transition_report = artifacts.get("actual_state_transition_report")
    if isinstance(expected_state_transition_report_path, str) and expected_state_transition_report_path:
        report_path = pack_dir / expected_state_transition_report_path
        if should_write_expected(report_path, update, record):
            if actual_state_transition_report is None:
                issues.append(
                    f"{pack_dir}: expected_state_transition_report 대상인데 state_transition_report 결과가 없습니다 ({expected_state_transition_report_path})"
                )
            else:
                report_path.parent.mkdir(parents=True, exist_ok=True)
                report_path.write_text(
                    f"{actual_state_transition_report}\n",
                    encoding="utf-8",
                    newline="\n",
                )
...
def main() -> int:
    ...
    parser.add_argument(
        "--report-summary-only",
        action="store_true",
        help="when --report-out is set, omit successful case rows from pack reports",
    )
    ...
    report_summary_only = bool(args.report_summary_only and args.report_out)
    progress_path = str(os.environ.get(PROGRESS_ENV_KEY, "")).strip()
    progress_enabled = bool(progress_path)
    started_at = time.perf_counter()
    current_stage = "-"
    last_completed_stage = "-"
    current_pack_stage = "-"
    last_completed_pack_stage = "-"
    current_case_stage = "-"
    last_completed_case_stage = "-"
    ...
    def update_progress(status: str) -> None:
        ...
        write_progress_snapshot(
            ...
            current_stage=current_stage,
            last_completed_stage=last_completed_stage,
            current_pack=current_pack_stage,
            last_completed_pack=last_completed_pack_stage,
            current_case=current_case_stage,
            last_completed_case=last_completed_case_stage,
    ...
    def transition_stage(name: str) -> None:
        ...
        if not progress_enabled:
            return
        normalized = safe_stage_token(name) or "-"
    ...
    run_log_lines = ["python " + " ".join(sys.argv), "pack golden updated"]
    ...
    for pack_dir in packs:
        ...
        if report_summary_only:
            pack_report["total_case_count"] = len(cases)
            pack_report["case_count"] = 0
        case_file_changed = False
        for idx, case in enumerate(cases, 1):
            current_case_stage = str(idx)
            transition_stage(f"pack.{current_pack_stage}.run_case_{idx}")
            case_progress_hook = None
            if progress_enabled:
                def case_progress_hook(stage_name: str, *, _pack=current_pack_stage, _idx=idx) -> None:
                    transition_stage(f"pack.{_pack}.run_case_{_idx}.{safe_stage_token(stage_name)}")
            ok, expected, got, stderr, artifacts = run_case(
                ...
                case,
                run_policy,
                args.update or args.record,
                progress_hook=case_progress_hook,
            )
            complete_stage(f"pack.{current_pack_stage}.run_case_{idx}")
            last_completed_case_stage = str(idx)
            current_case_stage = "-"
            ...
            if keep_case_row:
                ...
                if report_summary_only:
                    pack_report["case_count"] += 1
        if (args.update or args.record) and case_file_changed:
            transition_stage(f"pack.{current_pack_stage}.write_golden")
            golden_path = pack_dir / "golden.jsonl"
            lines = [json.dumps(case, ensure_ascii=False) for case in cases]
            golden_path.write_text("\n".join(lines) + "\n", encoding="utf-8", newline="\n")
            updated_pack_files += 1
            complete_stage(f"pack.{current_pack_stage}.write_golden")
        if args.update and pack_report["ok"]:
            transition_stage(f"pack.{current_pack_stage}.write_metadata")
            write_pack_metadata(pack_dir, cases, run_log_lines)
            complete_stage(f"pack.{current_pack_stage}.write_metadata")
        pack_report["elapsed_ms"] = int((time.perf_counter() - pack_started) * 1000)
        report_packs.append(pack_report)
    ...
    if args.report_out:
        ...
        report = {
            ...
            "generated_at_utc": datetime.now(timezone.utc).isoformat(),
            "mode": mode,
            "report_detail_mode": "summary_only" if report_summary_only else "full",
