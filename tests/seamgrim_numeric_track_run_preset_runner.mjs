...
const OK = "seamgrim_numeric_track_run_preset: ok";
...
async function main() {
  ...
  try {
    ...
    await page.waitForFunction(() => window.__SEAMGRIM_NUMERIC_TRACK_RUN_PRESET__?.schema === "seamgrim.numeric_track_run_preset.v1");

    const state = await page.evaluate(() => {
      const rail = document.querySelector("[data-run-preset-rail]");
      const chip = document.querySelector("[data-run-preset-numeric-track]");
      return {
        railNumeric: rail?.dataset?.numericTrack ?? "",
        chipText: chip?.textContent?.trim() ?? "",
        chipHidden: chip?.classList?.contains("hidden") ?? true,
        chipValue: chip?.dataset?.value ?? "",
        runPreset: window.__SEAMGRIM_RUN_PRESET_RAIL__ ?? null,
        numericPreset: window.__SEAMGRIM_NUMERIC_TRACK_RUN_PRESET__ ?? null,
        numericPresetText: window.__SEAMGRIM_NUMERIC_TRACK_RUN_PRESET_TEXT__ ?? "",
      };
    });
    ...
    assert(state.numericPreset?.module_ids?.includes("root_finding"), "numeric module root_finding missing");
    assert(String(state.numericPresetText).includes("schema\tseamgrim.numeric_track_run_preset.v1"), "numeric preset text schema missing");
    assert(String(state.numericPresetText).includes("recommended_views\tgraph|table"), "numeric preset text views missing");
