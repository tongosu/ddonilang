#!/usr/bin/env python3
"""Validate ROADMAP_V2_POST_PA1_FRONTIER_REBASE_V1."""
...
REPORT = ROOT / "docs" / "status" / "roadmap_v2" / "POST_PA1_FRONTIER_REBASE_REPORT_20260609.md"
...
PACK = ROOT / "pack" / "roadmap_v2_post_pa1_frontier_rebase_v1"
CONTRACT = PACK / "contract.detjson"
NEXT_FRONTIER = PACK / "next_frontier.detjson"


def fail(message: str) -> None:
    print(f"[roadmap-v2-post-pa1-frontier-rebase] FAIL: {message}", file=sys.stderr)
    raise SystemExit(1)
...
def check_files() -> None:
    for path in [
        ...
        CONTRACT,
        NEXT_FRONTIER,
        ROOT / "docs" / "status" / "roadmap_v2" / "하-0_REPORT_20260509.md",
        ROOT / "pack" / "education_curriculum_template_v1",
        ROOT / "tests" / "run_education_curriculum_template_check.py",
        ROOT / "tests" / "run_seamgrim_education_curriculum_template_check.py",
        ROOT / "tests" / "run_roadmap_v2_pa1_baseline_market_first_run_matrix_reconciliation_check.py",
    ]:
        if path.is_dir():
            continue
        require_file(path)
...
def check_docs() -> None:
    shared = [
        ...
        "HA0_EDUCATION_CURRICULUM_TEMPLATE_MATRIX_RECONCILIATION_V1",
        "ROADMAP_V2 post-PA1 frontier rebase 4/4 = 100%",
        ...
        "ha0_matrix_reconciliation_claim:false",
        "docs_ssot_change:false",
    ]
    for path in [REPORT, DEV_SUMMARY]:
        require_tokens(path, shared)
    require_tokens(PROJECT_STATUS, ["ROADMAP_V2_POST_PA1_FRONTIER_REBASE_V1", "55/90 = 61%", "59/90 = 66%", "하-0"])
    require_tokens(CHANGELOG, ["ROADMAP_V2 post-PA1 frontier rebase", "HA0_EDUCATION_CURRICULUM_TEMPLATE_MATRIX_RECONCILIATION_V1"])


def check_payloads() -> None:
    expected = {
        "schema": "ddn.pack.contract.v1",
        "pack": "roadmap_v2_post_pa1_frontier_rebase_v1",
        "kind": "roadmap_v2_post_pa1_frontier_rebase",
        "runtime_claim": False,
        "product_code_change": False,
        "product_ui_change": False,
        "education_runtime_change": False,
        "lesson_authoring_ui_change": False,
        "ssot_textbook_rewrite_claim": False,
        "closed_by": "ROADMAP_V2_POST_PA1_FRONTIER_REBASE_V1",
        "matrix_closure_claim": False,
        "matrix_closure_tier": "닫힘-문서",
        "roadmap_matrix_increment": False,
        "ha0_matrix_reconciliation_claim": False,
        "current_stage": "ROADMAP_V2 post-PA1 frontier rebase",
        ...
        "selected_next_work": "HA0_EDUCATION_CURRICULUM_TEMPLATE_MATRIX_RECONCILIATION_V1",
        "selected_coordinate": "하-0",
        "rejected_work": "NEW_LESSON_AUTHORING_UI_WITHOUT_HA0_RECONCILIATION",
        "docs_ssot_change": False,
    ...
    if frontier.get("schema") != "ddn.roadmap_v2.post_pa1_frontier_rebase.v1":
    ...
    for key in [
        ...
        "product_code_change",
        "product_ui_change",
        "education_runtime_change",
        "lesson_authoring_ui_change",
        "ssot_textbook_rewrite_claim",
        "matrix_closure_claim",
        "roadmap_matrix_increment",
        "ha0_matrix_reconciliation_claim",
        "docs_ssot_change",
    ]:
...
def check_frontier_basis() -> None:
    require_tokens(
        MATRIX,
        [
            "| 0마루 씨앗마루 | 총서/차시 템플릿 | 9권+참조권+교사용 | textbook map | 닫힘-동작 |",
            "| 2마루 닫힘마루 | 차시 pack 연결 | teacher_notes/student_sheet/pack | lesson pack PASS | 닫힘-동작 |",
            "| 5 | 하-0 | 교재 차시 템플릿/배움마당 기준 | textbook template pack | 닫힘-동작 |",
        ],
    )
    require_tokens(TRACKER, ["| 12 | `하-0` | 교재 차시 템플릿/배움마당 기준 | 닫힘-동작 |", "| 44 | `하-2` | 차시 pack 연결 | 닫힘-동작 |"])
    require_tokens(MANIFEST, ["| `하-0` | `education_curriculum_template_v1`; `roadmap_v2_ha0_education_curriculum_template_matrix_reconciliation_v1` |", "UI: `python tests/run_seamgrim_education_curriculum_template_check.py`"])
...
def check_forbidden_claims() -> None:
    forbidden = [
        ...
        '"runtime_claim": true',
        '"ha0_matrix_reconciliation_claim": true',
    ]
...
def check_gates() -> None:
    run([sys.executable, "tests/run_pack_golden.py", "roadmap_v2_post_pa1_frontier_rebase_v1"], timeout=120)
    run([sys.executable, "tests/run_roadmap_v2_pa1_baseline_market_first_run_matrix_reconciliation_check.py"], timeout=600)
...
def main() -> None:
    ...
    check_gates()
    print("[roadmap-v2-post-pa1-frontier-rebase] OK")
