...
PACK_SCHEMA = "ddn.lifecycle_pan_replay_minimum.pack.contract.v1"
RUN_CASES = {
    "c01_active_pan_replay": ["run", "tools/teul-cli/tests/golden/W111/W111_G10_lifecycle_transition_call_replays_active_pan/main.ddn"],
...
SMOKE_CASE = ("c03_state_view_boundary", "smoke_with_view_boundary.v1.json")


def fail(code: str, msg: str) -> int:
    print(f"[lifecycle-pan-replay-minimum-pack-check] fail code={code} msg={msg}", file=sys.stderr)
    return 1
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Lifecycle pan replay minimum pack checker")
    parser.add_argument("--pack", default="pack/lifecycle_pan_replay_minimum_v1")
    ...
    if missing:
        return fail("E_LIFECYCLE_PAN_REPLAY_FILE_MISSING", ",".join(missing))

    try:
        contract = load_json(pack / "contract.detjson")
        golden = load_jsonl(pack / "golden.jsonl")
    except ValueError as exc:
        return fail("E_LIFECYCLE_PAN_REPLAY_INVALID", str(exc))

    if not isinstance(contract, dict):
        return fail("E_LIFECYCLE_PAN_REPLAY_CONTRACT_TYPE", "contract root must be object")
    if str(contract.get("schema", "")).strip() != PACK_SCHEMA:
        return fail("E_LIFECYCLE_PAN_REPLAY_SCHEMA", f"schema={contract.get('schema')}")
    if str(contract.get("evidence_tier", "")).strip() != "runner_fill":
        return fail("E_LIFECYCLE_PAN_REPLAY_TIER", f"tier={contract.get('evidence_tier')}")
    if str(contract.get("closure_claim", "")).strip() != "no":
        return fail("E_LIFECYCLE_PAN_REPLAY_CLOSURE", f"closure={contract.get('closure_claim')}")
    for rel in contract.get("evidence_targets", []):
        if not Path(rel).exists():
            return fail("E_LIFECYCLE_PAN_REPLAY_TARGET", rel)

    golden_index = {str(row.get("id", "")).strip(): row for row in golden}
    if set(golden_index) != set(RUN_CASES) | {SMOKE_CASE[0]}:
        return fail("E_LIFECYCLE_PAN_REPLAY_GOLDEN_SET", f"golden={sorted(golden_index)}")
    ...
    for case_id, expected_cmd in RUN_CASES.items():
        ...
        if golden_row.get("cmd") != expected_cmd:
            return fail("E_LIFECYCLE_PAN_REPLAY_CMD", case_id)
        if golden_row.get("stdout") != expected.get("stdout"):
            return fail("E_LIFECYCLE_PAN_REPLAY_STDOUT", case_id)
        if int(golden_row.get("exit_code", -1)) != 0:
            return fail("E_LIFECYCLE_PAN_REPLAY_EXIT", case_id)
    ...
    if str(smoke_row.get("smoke_golden", "")).strip() != SMOKE_CASE[1]:
        return fail("E_LIFECYCLE_PAN_REPLAY_SMOKE", SMOKE_CASE[0])
    if str(smoke_expected.get("target", "")).strip() != SMOKE_CASE[1]:
        return fail("E_LIFECYCLE_PAN_REPLAY_SMOKE_EXPECTED", SMOKE_CASE[0])

    print("[lifecycle-pan-replay-minimum-pack-check] ok")
