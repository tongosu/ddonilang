...
def main() -> int:
    ...
    if not bool(row.get("ok")):
        print(
            "check=seamgrim_generic_imja_info_shorthand_raw_wasm "
    ...
    if not str(row.get("state_hash", "")).strip():
        print("check=seamgrim_generic_imja_info_shorthand_raw_wasm detail=missing_hash")
        return 1
    columns = [str(key) for key in row.get("columns", [])]
    if "체력" not in columns:
        print(
            "check=seamgrim_generic_imja_info_shorthand_raw_wasm "
            f"detail=missing_column:체력:{','.join(columns)}"
        )
        return 1
    print("seamgrim generic imja info shorthand raw wasm check ok columns=1")
    return 0
