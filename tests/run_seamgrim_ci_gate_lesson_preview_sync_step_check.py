...
FILE_TOKEN_REQUIREMENTS: dict[str, tuple[str, ...]] = {
    "tests/run_seamgrim_ci_gate.py": (
        '"lesson_preview_sync"',
        "tests/run_seamgrim_lesson_preview_sync_check.py",
        "--require-preview-synced",
        '"seamgrim_ci_gate_lesson_preview_sync_step_check"',
        "tests/run_seamgrim_ci_gate_lesson_preview_sync_step_check.py",
        '"seamgrim_ci_gate_lesson_preview_sync_step_check_selftest"',
        "tests/run_seamgrim_ci_gate_lesson_preview_sync_step_check_selftest.py",
    ),
    "tests/_seamgrim_ci_diag_lib.py": (
        'elif name == "lesson_preview_sync":',
        "lesson_preview_sync_failed",
        "lesson_preview_sync_detail",
        'elif name == "seamgrim_ci_gate_lesson_preview_sync_step_check":',
        "lesson_preview_sync_step_check_failed",
        "lesson_preview_sync_step_check_token_missing",
        'elif name == "seamgrim_ci_gate_lesson_preview_sync_step_check_selftest":',
        "lesson_preview_sync_step_selftest_failed",
    ),
...
def build_report(*, root: Path, missing: list[str]) -> dict[str, object]:
    return {
        "schema": "ddn.seamgrim_ci_gate_lesson_preview_sync_step_check.v1",
        "status": "pass" if not missing else "fail",
        "ok": not missing,
        "code": "OK" if not missing else "E_SEAMGRIM_LESSON_PREVIEW_SYNC_STEP_CONTRACT_MISSING",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check lesson_preview_sync wiring across seamgrim ci gate flow")
    parser.add_argument(
        "--repo-root",
    ...
    if missing:
        print("seamgrim ci gate lesson preview sync step check failed:")
        for token in missing[:12]:
            print(f" - missing token: {token}")
        return 1

    print("seamgrim ci gate lesson preview sync step check ok")
