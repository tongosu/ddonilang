...
def fail(detail: str) -> int:
    print(f"check=seamgrim_platform_mock_payload_snapshot detail={detail}")
    return 1


def main() -> int:
    root = Path(__file__).resolve().parent.parent
    runner = root / "tests" / "seamgrim_platform_mock_payload_snapshot_runner.mjs"
    ...
    print(f"[payload-runner] {runner.name} elapsed_ms={elapsed_ms}")
    print(detail or "seamgrim platform mock payload snapshot check ok")
    return 0
