...
PACK = ROOT / "pack" / "proof_certificate_v1_signed_emit_profiles_v1"
...
def validate_profile(work: Path, profile: str) -> None:
    ...
    cert_path = work / f"{profile}.proof.cert_manifest.detjson"
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="proof_certificate_v1_signed_emit_profiles_pack_") as td:
        work = Path(td)
        shutil.copytree(PACK / "fixtures", work / "fixtures")
        run(
            [str(exe), "cert", "keygen", "--out", "cert", "--seed", "signed-emit-profile-pack"],
            cwd=work,
        )
    ...
    print("proof_certificate_v1_signed_emit_profiles_pack: PASS")
    return 0
