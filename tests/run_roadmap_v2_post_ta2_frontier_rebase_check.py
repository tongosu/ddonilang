#!/usr/bin/env python3
"""Validate ROADMAP_V2_POST_TA2_FRONTIER_REBASE_V1."""
...
DOC = ROOT / "ROADMAP_V2_POST_TA2_FRONTIER_REBASE_V1.md"
REPORT = ROOT / "docs" / "status" / "roadmap_v2" / "POST_TA2_FRONTIER_REBASE_REPORT_20260608.md"
...
PACK = ROOT / "pack" / "roadmap_v2_post_ta2_frontier_rebase_v1"
CONTRACT = PACK / "contract.detjson"
NEXT_FRONTIER = PACK / "next_frontier.detjson"


def fail(message: str) -> None:
    print(f"[roadmap-v2-post-ta2-frontier-rebase] FAIL: {message}", file=sys.stderr)
    raise SystemExit(1)
...
def check_files() -> None:
    for path in [
        ...
        CONTRACT,
        NEXT_FRONTIER,
        ROOT / "ROADMAP_V2_TA2_GUIDE_STATUS_RECONCILIATION_V1.md",
        ROOT / "tests" / "run_roadmap_v2_ta2_guide_status_reconciliation_check.py",
        ROOT / "pack" / "math_vector_minimum_first_run_v1" / "README.md",
        ROOT / "pack" / "math_vector_minimum_first_run_v1" / "input.ddn",
...
def check_docs() -> None:
    shared = [
        "ROADMAP_V2_POST_TA2_FRONTIER_REBASE_V1",
        "ROADMAP_V2_DA1_MATH_FIRST_RUN_FRONTIER_REBASE_V1",
        "ROADMAP_V2 post-TA2 frontier rebase 4/4 = 100%",
        ...
        "legacy_checker_unlock_claim:false",
        "docs_ssot_change:false",
    ]
    for path in [DOC, REPORT, DEV_SUMMARY]:
        require_tokens(path, shared)
    require_tokens(PROJECT_STATUS, ["ROADMAP_V2_POST_TA2_FRONTIER_REBASE_V1", "45/90 = 50%", "59/90 = 66%", "다-1"])
    require_tokens(CHANGELOG, ["ROADMAP_V2 post-TA2 frontier rebase", "ROADMAP_V2_DA1_MATH_FIRST_RUN_FRONTIER_REBASE_V1"])
...
def check_payloads() -> None:
    ...
    expected_contract = {
        ...
        "pack": "roadmap_v2_post_ta2_frontier_rebase_v1",
        "kind": "roadmap_v2_post_ta2_frontier_rebase",
        "runtime_claim": False,
        "product_code_change": False,
        "product_ui_change": False,
        "closed_by": "ROADMAP_V2_POST_TA2_FRONTIER_REBASE_V1",
        "matrix_closure_claim": False,
        ...
        "guide_status_change": False,
        "legacy_checker_unlock_claim": False,
        "current_stage": "ROADMAP_V2 post-TA2 frontier rebase",
        ...
        "selected_next_work": "ROADMAP_V2_DA1_MATH_FIRST_RUN_FRONTIER_REBASE_V1",
        "selected_coordinate": "다-1",
        "rejected_work": "DA1_MATRIX_CLOSURE_WITHOUT_FRONTIER_REBASE",
    ...
    if frontier.get("schema") != "ddn.roadmap_v2.post_ta2_frontier_rebase.v1":
        fail("next_frontier schema mismatch")
    if frontier.get("selected_next_work") != "ROADMAP_V2_DA1_MATH_FIRST_RUN_FRONTIER_REBASE_V1":
        fail("next_frontier selected_next_work mismatch")
    if frontier.get("selected_coordinate") != "다-1":
...
def check_da1_frontier_basis() -> None:
    matrix = read(MATRIX)
    if (
        "| 1마루 첫실행마루 | exact/vector/function 첫실행 | 정확한 수, 벡터, 함수 그래프 | math smoke pack | 진행 |"
        not in matrix
        and "| 1마루 첫실행마루 | exact/vector/function 첫실행 | 정확한 수, 벡터, 함수 그래프 | math smoke pack | 닫힘-동작 |"
        not in matrix
    ):
        fail("다-1 matrix row should remain 진행 or be reconciled by next work")
    guide = read(GUIDE)
    for token in [
        "#### 다-1 — exact/vector/function 첫실행",
        "| 현재 상태 | 닫힘-동작 |",
        "`math_symbolic_proof_1_v1`",
        "다-1 math_function",
    ]:
        if token not in guide:
            fail(f"guide missing 다-1 basis token: {token}")
...
def check_gates() -> None:
    run([sys.executable, "tests/run_pack_golden.py", "roadmap_v2_post_ta2_frontier_rebase_v1"], timeout=120)
    run([sys.executable, "tests/run_roadmap_v2_ta2_guide_status_reconciliation_check.py"], timeout=600)
    run([sys.executable, "tests/run_pack_golden.py", "math_vector_minimum_first_run_v1"], timeout=120)
...
def main() -> None:
    ...
    check_docs()
    check_payloads()
    check_da1_frontier_basis()
    check_forbidden_claims()
    check_gates()
    print("[roadmap-v2-post-ta2-frontier-rebase] OK")
