...
def fail(detail: str) -> int:
    print(f"check=seamgrim_platform_mock_adapter_roundtrip detail={detail}")
    return 1


def main() -> int:
    root = Path(__file__).resolve().parent.parent
    runner = root / "tests" / "seamgrim_platform_mock_adapter_roundtrip_runner.mjs"
    ...
    print(f"[roundtrip-runner] {runner.name} elapsed_ms={elapsed_ms}")
    print(detail or "seamgrim platform mock adapter roundtrip check ok")
    return 0
