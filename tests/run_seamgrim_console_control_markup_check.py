...
PACK = ROOT / "pack" / "seamgrim_console_control_markup_v1"
...
def fail(message: str) -> int:
    print(f"[seamgrim-console-control-markup] fail: {message}", file=sys.stderr)
    return 1
...
def main() -> int:
    ...
    if contract.get("schema") != "ddn.seamgrim_console_control_markup.contract.v1":
    ...
    if "줄" in names or contract["policy"].get("line_control_command") != "행제어":
        return fail("line control command must be 행제어, not 줄")
    ...
    report = {
        "schema": "ddn.seamgrim_console_control_markup.report.v1",
        "ok": True,
    ...
    print("[seamgrim-console-control-markup] ok")
    return 0
