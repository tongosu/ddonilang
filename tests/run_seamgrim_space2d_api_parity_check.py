...
def run_export_space2d(root: Path, input_path: Path) -> dict:
    exporter = root / "solutions" / "seamgrim_ui_mvp" / "tools" / "export_space2d.py"
    with tempfile.TemporaryDirectory(prefix="seamgrim_space2d_api_parity_export_") as tmp:
        out_path = Path(tmp) / "space2d.json"
        ...
        if proc.returncode != 0:
            raise RuntimeError(proc.stderr.strip() or proc.stdout.strip() or "export_space2d failed")
        return json.loads(out_path.read_text(encoding="utf-8"))


def main() -> int:
    parser = argparse.ArgumentParser(description="Check export_space2d and ddn_exec_server space2d parity")
    parser.add_argument("--pack", default="pack/seamgrim_space2d_v0_basics")
    ...
    try:
        server_module, base_url, started_proc = start_parity_server(
            root=root,
            module_name="seamgrim_ddn_exec_server_check_for_space2d",
            host=args.host,
            port=int(args.port),
    ...
    try:
        summaries: list[dict] = []
        failures: list[str] = []
        for case in load_cases(pack_dir):
            ...
            with ThreadPoolExecutor(max_workers=2) as case_pool:
                standalone_future = case_pool.submit(run_export_space2d, root, input_path)
            ...
            doc_match = canonical_json(standalone) == canonical_json(api_doc)
            schema_match = standalone.get("schema") == "seamgrim.space2d.v0" and api_doc.get("schema") == "seamgrim.space2d.v0"
            ...
            camera_match = canonical_json(standalone.get("camera")) == canonical_json(api_doc.get("camera"))
        ...
        report = {
            "schema": "ddn.seamgrim_space2d_api_parity.v1",
            ...
            "camera_match_count": sum(1 for item in summaries if item.get("camera_match")),
            "cases": summaries,
        }
        if args.out:
        ...
        if failures:
            ...
            return 1
        print(
            "seamgrim space2d api parity ok "
