#!/usr/bin/env python3
"""Validate ROADMAP_V2_POST_TA0_FRONTIER_REBASE_V1."""
...
DOC = ROOT / "ROADMAP_V2_POST_TA0_FRONTIER_REBASE_V1.md"
REPORT = ROOT / "docs" / "status" / "roadmap_v2" / "POST_TA0_FRONTIER_REBASE_REPORT_20260608.md"
DEV_SUMMARY = ROOT / "docs" / "context" / "all" / "DEV_SUMMARY.md"
PROJECT_STATUS = ROOT / "docs" / "status" / "PROJECT_STATUS.md"
CHANGELOG = ROOT / "docs" / "status" / "CHANGELOG.md"
MATRIX = ROOT / "docs" / "context" / "roadmap" / "ROADMAP_V2_GANADA_MILESTONE_MATRIX_20260426.md"
PACK = ROOT / "pack" / "roadmap_v2_post_ta0_frontier_rebase_v1"
CONTRACT = PACK / "contract.detjson"
NEXT_FRONTIER = PACK / "next_frontier.detjson"


def fail(message: str) -> None:
    print(f"[roadmap-v2-post-ta0-frontier-rebase] FAIL: {message}", file=sys.stderr)
    raise SystemExit(1)
...
def check_files() -> None:
    for path in [
        ...
        CONTRACT,
        NEXT_FRONTIER,
        ROOT / "ROADMAP_V2_TA0_PACK_CHECKER_SKELETON_MATRIX_RECONCILIATION_V1.md",
        ROOT / "tests" / "run_roadmap_v2_ta0_pack_checker_skeleton_matrix_reconciliation_check.py",
...
def check_docs() -> None:
    shared = [
        ...
        "ROADMAP_V2 post-TA0 frontier rebase 4/4 = 100%",
    ...
    require_tokens(PROJECT_STATUS, ["ROADMAP_V2_POST_TA0_FRONTIER_REBASE_V1", "44/90 = 49%", "59/90 = 66%", "타-1"])
    require_tokens(CHANGELOG, ["ROADMAP_V2 post-TA0 frontier rebase", "ROADMAP_V2_TA1_PACK_RUNNER_BASIS_MATRIX_RECONCILIATION_V1"])
...
def check_payloads() -> None:
    ...
    expected_contract = {
        ...
        "pack": "roadmap_v2_post_ta0_frontier_rebase_v1",
        "kind": "roadmap_v2_post_ta0_frontier_rebase",
        "runtime_claim": False,
        "product_code_change": False,
        "product_ui_change": False,
        "closed_by": "ROADMAP_V2_POST_TA0_FRONTIER_REBASE_V1",
        "matrix_closure_claim": False,
        "matrix_closure_tier": "닫힘-문서",
        "roadmap_matrix_increment": False,
        "current_stage": "ROADMAP_V2 post-TA0 frontier rebase",
        ...
        "selected_next_work": "ROADMAP_V2_TA1_PACK_RUNNER_BASIS_MATRIX_RECONCILIATION_V1",
        "selected_coordinate": "타-1",
    ...
    if frontier.get("schema") != "ddn.roadmap_v2.post_ta0_frontier_rebase.v1":
        fail("next_frontier schema mismatch")
    if frontier.get("selected_next_work") != "ROADMAP_V2_TA1_PACK_RUNNER_BASIS_MATRIX_RECONCILIATION_V1":
        fail("next_frontier selected_next_work mismatch")
    if frontier.get("selected_coordinate") != "타-1":
...
def check_matrix_candidate() -> None:
    for line in read(MATRIX).splitlines():
        if "| 1마루 첫실행마루 | runner PASS | 단일 pack runner | runner PASS |" in line:
            if "진행" not in line and "닫힘-동작" not in line:
                fail("타-1 matrix row should remain 진행 or be reconciled by TA1 closure")
            return
    fail("missing 타-1 matrix row")
...
def check_gates() -> None:
    run([sys.executable, "tests/run_pack_golden.py", "roadmap_v2_post_ta0_frontier_rebase_v1"], timeout=120)
    run([sys.executable, "tests/run_roadmap_v2_ta0_pack_checker_skeleton_matrix_reconciliation_check.py"], timeout=600)
...
def main() -> None:
    ...
    check_gates()
    print("[roadmap-v2-post-ta0-frontier-rebase] OK")
