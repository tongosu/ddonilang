...
const OK = "seamgrim_numeric_track_result_compare_history_report_table_status: ok";
...
async function main() {
  ...
  try {
    ...
    await page.waitForSelector(".numeric-track-compare-history-report-table-status");
    ...
    const result = await page.evaluate(() => ({
      ...
      statusDataset: document.querySelector(".numeric-track-compare-history-report-table-status")?.dataset.status ?? "",
      statusDomText: document.querySelector(".numeric-track-compare-history-report-table-status")?.textContent ?? "",
    }));

    assert(result.summary?.schema === "seamgrim.numeric_track_result_compare_history_report_table_summary.v1", "summary schema mismatch");
    assert(result.status.track_id === TRACK_ID, `status track mismatch: ${result.status.track_id}`);
    assert(result.status.source_schema === "seamgrim.numeric_track_result_compare_history_report_table_summary.v1", "status source schema mismatch");
    assert(result.status.status_claim === "metadata_status", `status claim mismatch: ${result.status.status_claim}`);
    assert(result.status.replay_claim === false, "status must not claim replay");
    assert(result.status.pair_count === 2, `status pair count mismatch: ${result.status.pair_count}`);
    assert(result.status.column_count === 8, `status column count mismatch: ${result.status.column_count}`);
    assert(result.status.lesson_count === 3, `status lesson count mismatch: ${result.status.lesson_count}`);
    assert(result.status.state_hash_changed_count === 2, `status hash count mismatch: ${result.status.state_hash_changed_count}`);
    assert(result.status.run_kind_changed_count === 1, `status run-kind count mismatch: ${result.status.run_kind_changed_count}`);
    assert(result.status.channel_delta_total === 1, `status channel total mismatch: ${result.status.channel_delta_total}`);
    assert(result.status.channel_delta_abs_total === 1, `status channel abs total mismatch: ${result.status.channel_delta_abs_total}`);
    assert(result.status.has_changes === true, "status should report changes");
    assert(result.status.status === "변화있음", `status value mismatch: ${result.status.status}`);
    assert(result.status.status_reasons?.join("|") === "state_hash_changed|run_kind_changed|channel_delta_nonzero", "status reasons mismatch");
    assert(result.status.summary?.schema === "seamgrim.numeric_track_result_compare_history_report_table_summary.v1", "embedded summary missing");
    assert(String(result.statusText).includes("schema\tseamgrim.numeric_track_result_compare_history_report_table_status.v1"), "status text schema missing");
    assert(String(result.statusText).includes("status_claim\tmetadata_status"), "status text claim missing");
    assert(String(result.statusText).includes("replay_claim\tfalse"), "status text replay boundary missing");
    assert(String(result.statusText).includes("status\t변화있음"), "status text value missing");
    assert(String(result.statusText).includes("status_reasons\tstate_hash_changed|run_kind_changed|channel_delta_nonzero"), "status text reasons missing");
    assert(result.statusDataset === "변화있음", `status dataset mismatch: ${result.statusDataset}`);
    assert(String(result.statusDomText).includes("상태 변화있음"), "status DOM value missing");
    assert(String(result.statusDomText).includes("이유 3"), "status DOM reason count missing");
