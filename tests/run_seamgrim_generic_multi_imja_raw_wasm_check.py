...
def main() -> int:
    ...
    if not bool(payload.get("ok")):
        print(
            "check=seamgrim_generic_multi_imja_raw_wasm "
    ...
    for idx, state in enumerate(states):
        ...
        if not str(state.get("state_hash", "")).strip():
            ...
            return 1
        player = value_at(state, "플레이어__체력")
        enemy = value_at(state, "적__체력")
        stage = value_at(state, "단계")
        if player is None or enemy is None or stage is None:
            print(
                "check=seamgrim_generic_multi_imja_raw_wasm "
    ...
    if player_values != ["99", "98", "97"]:
        print(
            "check=seamgrim_generic_multi_imja_raw_wasm "
            f"detail=bad_player_sequence:{','.join(player_values)}"
        )
        return 1
    if enemy_values != ["48", "46", "44"]:
        print(
            "check=seamgrim_generic_multi_imja_raw_wasm "
    ...
    if stage_values != ["1", "2", "3"]:
        ...
        return 1
    print("seamgrim generic multi imja raw wasm check ok player=99,98,97 enemy=48,46,44 stage=1,2,3")
    return 0
