#!/usr/bin/env python
from __future__ import annotations

import subprocess
from pathlib import Path


ROOT = Path(__file__).resolve().parents[1]
DOC = ROOT / "ROADMAP_V2_A2_NURIGYM_REPRESENTATIVE_ENV_REBASE_V1.md"
REPORT = ROOT / "docs" / "status" / "roadmap_v2" / "아-2_REBASE_20260604.md"
QUEUE = ROOT / "NEXT_WORK_QUEUE_AFTER_CONNECT_V1.md"


def fail(code: str, message: str) -> int:
    print(f"{code}: {message}")
    return 1


def read(path: Path) -> str:
    return path.read_text(encoding="utf-8")


def require_files() -> int:
    required = [
        DOC,
        REPORT,
        QUEUE,
        ROOT / "ROADMAP_V2_A1_NURIGYM_FINAL_CLOSURE_V1.md",
        ROOT / "tests" / "run_roadmap_v2_a1_final_closure_check.py",
        ROOT / "pack" / "gogae5_w47_nurigym_observation_spec" / "golden.jsonl",
        ROOT / "pack" / "nuri_gym_cartpole_v1" / "golden.jsonl",
        ROOT / "pack" / "nuri_gym_cartpole_shared_sync_v1" / "golden.jsonl",
        ROOT / "pack" / "nuri_gym_gridmaze_v1" / "golden.jsonl",
        ROOT / "pack" / "nuri_gym_gridworld_v1" / "golden.jsonl",
        ROOT / "pack" / "nuri_gym_pendulum_v1" / "golden.jsonl",
        ROOT / "pack" / "nuri_gym_canon_contract_v1" / "golden.jsonl",
        ROOT / "pack" / "nuri_gym_bandit_v1" / "golden.jsonl",
        ROOT / "tests" / "run_nuri_gym_contract_check.py",
        ROOT / "NURIGYM_CONTRACT_GRIDWORLD_EXPECTED_REFRESH_V1.md",
        ROOT / "NURIGYM_CARTPOLE_PENDULUM_EXPECTED_REFRESH_V1.md",
        ROOT / "NURIGYM_BANDIT_MINIMUM_PACK_V1.md",
        ROOT / "ROADMAP_V2_A2_NURIGYM_FINAL_CLOSURE_RECHECK_V1.md",
    ]
    missing = [str(path.relative_to(ROOT)) for path in required if not path.exists()]
    if missing:
        return fail("E_ROADMAP_V2_A2_REBASE_MISSING", str(missing))
    return 0


def require_tokens(path: Path, tokens: list[str], code: str) -> int:
    text = read(path)
    missing = [token for token in tokens if token not in text]
    if missing:
        return fail(code, f"{path.relative_to(ROOT)} missing {missing}")
    return 0


def check_doc() -> int:
    return require_tokens(
        DOC,
        [
            "ROADMAP_V2 `아-2`",
            "representative environment golden",
            "CartPole",
            "GridMaze",
            "Bandit",
            "dataset hash pack",
            "nuri_gym_canon_contract_v1",
            "nuri_gym_gridworld_v1",
            "nuri_gym_pendulum_v1",
            "nuri_gym_cartpole_shared_sync_v1",
            "No current `nuri_gym_bandit_*` pack exists",
            "python tests/run_pack_golden.py nuri_gym_canon_contract_v1 nuri_gym_gridworld_v1` fails",
            "sha256:34289c2282de6f9b8f4869a20fa48dff68ff4f3b49f2d0340178c321b2aad313",
            "sha256:062a37291f73fd6b2d09e50392f7228c56a4d030c0958a7ed050b3ecfd0e993a",
            "NURIGYM_CONTRACT_GRIDWORLD_EXPECTED_REFRESH_V1",
            "NURIGYM_CARTPOLE_PENDULUM_EXPECTED_REFRESH_V1",
            "NURIGYM_BANDIT_MINIMUM_PACK_V1",
            "ROADMAP_V2_A2_NURIGYM_FINAL_CLOSURE_RECHECK_V1",
            "closed ROADMAP_V2 `아-2` final closure",
            "does not claim",
            "Python/Web parity",
            "dataset registry",
            "training workflow",
        ],
        "E_ROADMAP_V2_A2_REBASE_DOC",
    )


def check_report() -> int:
    return require_tokens(
        REPORT,
        [
            "ROADMAP_V2 `아-2`",
            "representative environment golden",
            "nuri_gym_canon_contract_v1",
            "nuri_gym_gridworld_v1",
            "No `nuri_gym_bandit_*` pack exists",
            "NURIGYM_CONTRACT_GRIDWORLD_EXPECTED_REFRESH_V1",
            "NURIGYM_CARTPOLE_PENDULUM_EXPECTED_REFRESH_V1",
            "NURIGYM_BANDIT_MINIMUM_PACK_V1",
            "ROADMAP_V2_A2_NURIGYM_FINAL_CLOSURE_RECHECK_V1",
            "ROADMAP_V2 `아-2`: closed",
            "ROADMAP_V2 `아-2`: keep open",
            "docs/ssot",
        ],
        "E_ROADMAP_V2_A2_REBASE_REPORT",
    )


def check_bandit_pack() -> int:
    required = [
        ROOT / "pack" / "nuri_gym_bandit_v1" / "golden.jsonl",
        ROOT / "pack" / "nuri_gym_bandit_v1" / "golden.txt",
        ROOT / "pack" / "nuri_gym_bandit_v1" / "golden_alt.txt",
    ]
    missing = [str(path.relative_to(ROOT)) for path in required if not path.exists()]
    if missing:
        return fail(
            "E_ROADMAP_V2_A2_REBASE_BANDIT_PACK",
            str(missing),
        )
    return 0


def run_support_checks() -> int:
    command = [
        "python",
        "tests/run_pack_golden.py",
        "nuri_gym_gridmaze_v1",
        "nuri_gym_cartpole_shared_sync_v1",
        "gogae5_w47_nurigym_observation_spec",
    ]
    result = subprocess.run(
        command,
        cwd=ROOT,
        text=True,
        encoding="utf-8",
        errors="replace",
        stdout=subprocess.PIPE,
        stderr=subprocess.STDOUT,
    )
    if result.returncode != 0:
        return fail("E_ROADMAP_V2_A2_REBASE_SUPPORT", result.stdout.strip())
    return 0


def check_queue() -> int:
    text = read(QUEUE)
    required = [
        "ROADMAP_V2_A2_NURIGYM_REPRESENTATIVE_ENV_REBASE_V1",
        "closed by `ROADMAP_V2_A2_NURIGYM_REPRESENTATIVE_ENV_REBASE_V1.md`",
        "ROADMAP_V2 `아-2` remains open",
        "nuri_gym_canon_contract_v1",
        "nuri_gym_gridworld_v1",
        "NURIGYM_CONTRACT_GRIDWORLD_EXPECTED_REFRESH_V1",
        "NURIGYM_CARTPOLE_PENDULUM_EXPECTED_REFRESH_V1",
        "NURIGYM_BANDIT_MINIMUM_PACK_V1",
        "ROADMAP_V2_A2_NURIGYM_FINAL_CLOSURE_RECHECK_V1",
        "closed by `ROADMAP_V2_A2_NURIGYM_FINAL_CLOSURE_RECHECK_V1.md`",
        "ROOT_LOW_RISK_RETIRE_DELETE_V1",
        "Approval-gated",
        "docs/ssot/**",
    ]
    missing = [token for token in required if token not in text]
    if missing:
        return fail("E_ROADMAP_V2_A2_REBASE_QUEUE", str(missing))
    if "1. `ROADMAP_V2_A2_NURIGYM_REPRESENTATIVE_ENV_REBASE_V1`" in text:
        return fail(
            "E_ROADMAP_V2_A2_REBASE_STILL_OPEN",
            "ROADMAP_V2_A2_NURIGYM_REPRESENTATIVE_ENV_REBASE_V1 is still next",
        )
    if "1. `ROOT_LOW_RISK_RETIRE_DELETE_V1`" not in text:
        return fail(
            "E_ROADMAP_V2_A2_REBASE_NEXT",
            "ROOT_LOW_RISK_RETIRE_DELETE_V1 is not next",
        )
    return 0


def check_docs_ssot_clean() -> int:
    result = subprocess.run(
        ["git", "status", "--short", "--", "docs/ssot"],
        cwd=ROOT,
        text=True,
        encoding="utf-8",
        errors="replace",
        stdout=subprocess.PIPE,
        stderr=subprocess.STDOUT,
    )
    if result.returncode != 0:
        return fail("E_ROADMAP_V2_A2_REBASE_SSOT_STATUS", result.stdout.strip())
    if result.stdout.strip():
        return fail("E_ROADMAP_V2_A2_REBASE_SSOT_DIRTY", result.stdout.strip())
    return 0


def main() -> int:
    checks = (
        require_files,
        check_doc,
        check_report,
        check_bandit_pack,
        run_support_checks,
        check_queue,
        check_docs_ssot_clean,
    )
    for check in checks:
        rc = check()
        if rc:
            return rc
    print("[roadmap-v2-a2-nurigym-rebase-v1] OK")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
