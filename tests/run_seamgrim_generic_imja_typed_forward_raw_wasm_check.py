...
def main() -> int:
    ...
    if not bool(payload.get("ok")):
        print(
            "check=seamgrim_generic_imja_typed_forward_raw_wasm "
    ...
    for idx, state in enumerate(states):
        ...
        if not str(state.get("state_hash", "")).strip():
            ...
            return 1
        hp = value_at(state, "목표__체력")
        stage = value_at(state, "단계")
        if hp is None or stage is None:
            print(
                "check=seamgrim_generic_imja_typed_forward_raw_wasm "
    ...
    if stage_values != ["1", "2", "3"]:
        ...
        return 1
    print("seamgrim generic imja typed forward raw wasm check ok hp=7,4,1 stage=1,2,3")
    return 0
