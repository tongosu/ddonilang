...
AGE4_PROOF_MARKER_PREFIX = "[ci-profile-full] aggregate age4 proof ok="
...
PROFILE_MATRIX_FULL_REAL_SMOKE_STEP_TIMEOUT_DEFAULT_SEC = 1200.0
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run CI profile gate for full")
    parser.add_argument(
        "--quick",
        action="store_true",
    ...
    sync_profile_marker = "sanity_profile=full"
    ...
    temp_ctx = (
        tempfile.TemporaryDirectory(prefix="ci_profile_full_gate_")
    ...
    with temp_ctx as td:
        ...
        if proc.returncode != 0:
            print("ci_profile_full_status=fail reason=sanity_gate_failed")
            return proc.returncode
        if any(marker not in stdout for marker in sanity_required_markers):
            print("ci_profile_full_status=fail reason=pass_marker_missing")
            return 1
        ...
        if sync_proc.returncode != 0:
            print("ci_profile_full_status=fail reason=sync_readiness_failed")
            return sync_proc.returncode
        if "ci_sync_readiness_status=pass" not in sync_stdout or sync_profile_marker not in sync_stdout:
            print("ci_profile_full_status=fail reason=sync_readiness_pass_marker_missing")
            return 1
        ...
        if report_proc.returncode != 0:
            print("ci_profile_full_status=fail reason=sync_readiness_report_check_failed")
            return report_proc.returncode

        if use_profile_matrix_full_real_smoke:
            print("[ci-profile-full] profile-matrix full-real smoke enabled")
            if profile_matrix_full_real_smoke_step_timeout_sec > 0.0:
                print(
                    "[ci-profile-full] profile-matrix full-real smoke step timeout sec="
        ...
        if use_profile_matrix_full_real_smoke and (args.quick or truthy_env("DDN_CI_PROFILE_GATE_SKIP_AGGREGATE")):
            print("ci_profile_full_status=fail reason=profile_matrix_full_real_smoke_requires_aggregate")
            return 1
        if use_profile_matrix_full_real_smoke and not use_full_aggregate:
            print("ci_profile_full_status=fail reason=profile_matrix_full_real_smoke_requires_full_aggregate")
            return 1

        if args.quick or truthy_env("DDN_CI_PROFILE_GATE_SKIP_AGGREGATE"):
            if args.quick:
                print("[ci-profile-full] aggregate gate skipped by --quick")
            else:
                print("[ci-profile-full] aggregate gate skipped by DDN_CI_PROFILE_GATE_SKIP_AGGREGATE=1")
            print("ci_profile_full_status=pass")
            return 0

        if use_full_aggregate:
            print("[ci-profile-full] aggregate gate uses full mode")
        else:
            print("[ci-profile-full] aggregate gate uses contract-only mode")
        ...
        if aggregate_proc.returncode != 0:
            print("ci_profile_full_status=fail reason=aggregate_gate_failed")
        ...
        if index_proc.returncode != 0:
            print("ci_profile_full_status=fail reason=aggregate_index_contract_failed")
            return index_proc.returncode
        index_doc = load_json(index_report)
        if not isinstance(index_doc, dict):
            print("ci_profile_full_status=fail reason=aggregate_index_json_invalid")
            return 1
        reports = index_doc.get("reports")
        if not isinstance(reports, dict):
            print("ci_profile_full_status=fail reason=aggregate_index_reports_missing_or_invalid")
            return 1
        fixed64_threeway_inputs_raw = str(reports.get("fixed64_threeway_inputs", "")).strip()
        if not fixed64_threeway_inputs_raw:
            print("ci_profile_full_status=fail reason=aggregate_fixed64_threeway_inputs_path_missing")
            return 1
        fixed64_threeway_inputs_path = Path(fixed64_threeway_inputs_raw.replace("\\", "/"))
        if not fixed64_threeway_inputs_path.exists():
            print("ci_profile_full_status=fail reason=aggregate_fixed64_threeway_inputs_report_missing")
            return 1
        fixed64_threeway_inputs_doc = load_json(fixed64_threeway_inputs_path)
        if not isinstance(fixed64_threeway_inputs_doc, dict):
            print("ci_profile_full_status=fail reason=aggregate_fixed64_threeway_inputs_report_invalid")
            return 1
        if str(fixed64_threeway_inputs_doc.get("schema", "")).strip() != "ddn.fixed64.threeway_inputs.v1":
            print("ci_profile_full_status=fail reason=aggregate_fixed64_threeway_inputs_schema_mismatch")
            return 1
        ...
        if summary_proc.returncode != 0:
            print("ci_profile_full_status=fail reason=aggregate_summary_contract_failed")
            return summary_proc.returncode
        if not summary_report.exists():
            print("ci_profile_full_status=fail reason=aggregate_summary_missing")
            return 1
        summary_kv = parse_summary(summary_report)
        if summary_kv.get("ci_sanity_gate_profile", "") != profile:
            print("ci_profile_full_status=fail reason=aggregate_summary_sanity_profile_mismatch")
            return 1
        if summary_kv.get("ci_sync_readiness_sanity_profile", "") != profile:
            print("ci_profile_full_status=fail reason=aggregate_summary_sync_profile_mismatch")
            return 1
        if summary_kv.get("ci_sanity_pack_golden_lang_consistency_ok", "") != "1":
            print("ci_profile_full_status=fail reason=aggregate_summary_lang_consistency_mismatch")
            return 1
        if summary_kv.get("ci_sanity_pack_golden_metadata_ok", "") != "1":
            print("ci_profile_full_status=fail reason=aggregate_summary_pack_golden_metadata_mismatch")
            return 1
        if summary_kv.get("ci_sanity_pack_golden_graph_export_ok", "") != "1":
            print("ci_profile_full_status=fail reason=aggregate_summary_pack_golden_graph_export_mismatch")
            return 1
        if summary_kv.get("ci_sync_readiness_ci_sanity_pack_golden_graph_export_ok", "") != "1":
            print("ci_profile_full_status=fail reason=aggregate_summary_sync_pack_golden_graph_export_mismatch")
            return 1
        if summary_kv.get("ci_sanity_seamgrim_numeric_factor_policy_ok", "") != "1":
            print("ci_profile_full_status=fail reason=aggregate_summary_sanity_numeric_factor_policy_mismatch")
            return 1
        if summary_kv.get("ci_sync_readiness_ci_sanity_seamgrim_numeric_factor_policy_ok", "") != "1":
            print("ci_profile_full_status=fail reason=aggregate_summary_sync_numeric_factor_policy_mismatch")
            return 1
        if summary_kv.get("ci_sanity_canon_ast_dpack_ok", "") != "1":
            print("ci_profile_full_status=fail reason=aggregate_summary_canon_ast_dpack_mismatch")
            return 1
        if not age4_proof_summary_ok(summary_kv):
            print("ci_profile_full_status=fail reason=aggregate_summary_age4_proof_mismatch")
            return 1
        ...
        if not runtime_helper_summary_ok(summary_kv):
            print("ci_profile_full_status=fail reason=aggregate_summary_runtime_helper_contract_mismatch")
            return 1
        maybe_force_group_id_summary_mismatch(summary_kv)
        if not group_id_summary_ok(summary_kv):
            print("ci_profile_full_status=fail reason=aggregate_summary_group_id_summary_mismatch")
            return 1
        if not profile_matrix_summary_ok(summary_kv, profile, FULL_PROFILE_MATRIX_VALUES):
            print("ci_profile_full_status=fail reason=aggregate_summary_profile_matrix_contract_mismatch")
            return 1
        print("[ci-profile-full] aggregate summary sanity markers ok")
        ...
        if emit_artifacts_proc.returncode != 0:
            print("ci_profile_full_status=fail reason=aggregate_emit_artifacts_contract_failed")
        ...
        if use_profile_matrix_full_real_smoke:
            ...
            if smoke_proc.returncode != 0:
                print("ci_profile_full_status=fail reason=profile_matrix_full_real_smoke_failed")
                return smoke_proc.returncode
            if PROFILE_MATRIX_FULL_REAL_SMOKE_STATUS_PASS not in smoke_stdout:
                print("ci_profile_full_status=fail reason=profile_matrix_full_real_smoke_marker_missing")
                return 1
