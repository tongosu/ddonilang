#!/usr/bin/env python3
"""Validate ROADMAP_V2_TA2_GUIDE_STATUS_RECONCILIATION_V1."""
...
DOC = ROOT / "ROADMAP_V2_TA2_GUIDE_STATUS_RECONCILIATION_V1.md"
REPORT = ROOT / "docs" / "status" / "roadmap_v2" / "타-2_GUIDE_RECONCILIATION_REPORT_20260608.md"
...
PACK = ROOT / "pack" / "roadmap_v2_ta2_guide_status_reconciliation_v1"
CONTRACT = PACK / "contract.detjson"
RECONCILIATION = PACK / "reconciliation.detjson"


def fail(message: str) -> None:
    print(f"[roadmap-v2-ta2-guide-reconciliation] FAIL: {message}", file=sys.stderr)
    raise SystemExit(1)
...
def check_files_and_docs() -> None:
    for path in [
        ...
        CONTRACT,
        RECONCILIATION,
        ROOT / "ROADMAP_V2_POST_TA1_FRONTIER_REBASE_V1.md",
        ROOT / "tests" / "run_roadmap_v2_post_ta1_frontier_rebase_check.py",
        ROOT / "docs" / "status" / "roadmap_v2" / "타-2_RECONCILIATION_REPORT_20260608.md",
        ROOT / "pack" / "roadmap_v2_ta2_matrix_status_reconciliation_v1" / "contract.detjson",
    ...
    shared_tokens = [
        ...
        "TA2 guide status reconciliation 4/4 = 100%",
    ...
    require_tokens(PROJECT_STATUS, ["ROADMAP_V2_TA2_GUIDE_STATUS_RECONCILIATION_V1", "45/90 = 50%", "59/90 = 66%"])
    require_tokens(CHANGELOG, ["ROADMAP_V2 TA2 guide status reconciliation", "ROADMAP_V2_POST_TA2_FRONTIER_REBASE_V1"])
    require_tokens(
        MATRIX,
    ...
    require_tokens(
        ...
        [
            "#### 타-2",
            "| 현재 상태 | 닫힘-동작 |",
            "| pack 후보 | `toolchain_pack_2_v1`; `roadmap_v2_ta2_matrix_status_reconciliation_v1`; `roadmap_v2_ta2_guide_status_reconciliation_v1` |",
        ],
    )
    require_tokens(TRACKER, ["| 7 | `타-2` | work item evidence gate 재정렬 | 닫힘-동작 |", "타-2_GUIDE_RECONCILIATION_REPORT_20260608.md"])
    require_tokens(
        MANIFEST,
        [
            "| `타-2` | `toolchain_pack_2_v1`; `roadmap_v2_ta2_matrix_status_reconciliation_v1`; `roadmap_v2_ta2_guide_status_reconciliation_v1` |",
            "python tests/run_roadmap_v2_ta2_guide_status_reconciliation_check.py",
            "authoritative matrix/guide status",
        ],
    )
...
def check_contracts() -> None:
    ...
    expected = {
        ...
        "pack": "roadmap_v2_ta2_guide_status_reconciliation_v1",
        "kind": "roadmap_v2_ta2_guide_status_reconciliation",
        "runtime_claim": False,
        ...
        "closed_by": "ROADMAP_V2_TA2_GUIDE_STATUS_RECONCILIATION_V1",
        "roadmap_coordinate": "타-2",
        "current_stage": "TA2 guide status reconciliation",
        "next_item": "ROADMAP_V2_POST_TA2_FRONTIER_REBASE_V1",
    ...
    record = reconciliation.get("guide_status_record", {})
    if record.get("previous_status") != "진행" or record.get("new_status") != "닫힘-동작":
        fail("missing guide status record")
    if reconciliation.get("matrix_closure_claim") is not False:
        fail("reconciliation must not claim new matrix closure")
...
def check_gates() -> None:
    run([sys.executable, "tests/run_pack_golden.py", "roadmap_v2_ta2_guide_status_reconciliation_v1"], timeout=120)
    run([sys.executable, "tests/run_roadmap_v2_post_ta1_frontier_rebase_check.py"], timeout=600)
...
def main() -> None:
    ...
    check_gates()
    print("[roadmap-v2-ta2-guide-reconciliation] OK")
