...
const OK = "seamgrim_numeric_track_result_compare_history_export: ok";
...
async function main() {
  ...
  try {
    ...
    await page.waitForSelector("#btn-copy-numeric-track-result-compare-history-export");
    await page.click("#btn-copy-numeric-track-result-compare-history-export");
    await page.waitForFunction(() => window.__SEAMGRIM_NUMERIC_TRACK_RESULT_COMPARE_HISTORY_EXPORT__?.schema === "seamgrim.numeric_track_result_compare_history_export.v1");

    const result = await page.evaluate(() => ({
      historyExport: window.__SEAMGRIM_NUMERIC_TRACK_RESULT_COMPARE_HISTORY_EXPORT__,
      text: window.__SEAMGRIM_NUMERIC_TRACK_RESULT_COMPARE_HISTORY_EXPORT_TEXT__,
    ...
    assert(result.historyExport.track_id === TRACK_ID, `track mismatch: ${result.historyExport.track_id}`);
    assert(result.historyExport.source_schema === "seamgrim.numeric_track_result_compare_history.v1", "export source schema mismatch");
    assert(result.historyExport.export_claim === "metadata_text", `export claim mismatch: ${result.historyExport.export_claim}`);
    assert(result.historyExport.replay_claim === false, "history export must not claim replay");
    assert(result.historyExport.pair_count === 2, `export pair count mismatch: ${result.historyExport.pair_count}`);
    assert(result.historyExport.history?.schema === "seamgrim.numeric_track_result_compare_history.v1", "embedded history missing");
    assert(String(result.historyExport.history_text).includes("pair_count\t2"), "embedded history text pair count missing");
    assert(String(result.text).includes("schema\tseamgrim.numeric_track_result_compare_history_export.v1"), "export text schema missing");
    assert(String(result.text).includes("source_schema\tseamgrim.numeric_track_result_compare_history.v1"), "export text source schema missing");
    ...
    assert(String(result.text).includes("history_text"), "export text history section missing");
    assert(String(result.text).includes(`0\t${ECON_ID}\t${PHYS_ID}`), "export text first pair missing");
    assert(String(result.text).includes(`1\t${PHYS_ID}\t${MATH_ID}`), "export text second pair missing");
    assert(String(result.panelText).includes("이력 복사"), "copy action missing");
