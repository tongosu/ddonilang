...
AGE4_PROOF_MARKER_PREFIX = "[ci-profile-core-lang] aggregate age4 proof ok="
...
PROFILE_MATRIX_FULL_REAL_SMOKE_STEP_TIMEOUT_DEFAULT_SEC = 900.0


CORE_LANG_PROFILE_MATRIX_SUMMARY_KEYS = (
...
def profile_matrix_summary_ok(summary_kv: dict[str, str], profile: str, expected_values: str) -> bool:
    return (
        ...
        and summary_kv.get(CORE_LANG_PROFILE_MATRIX_SUMMARY_KEYS[0], "") == "pass"
        and summary_kv.get(CORE_LANG_PROFILE_MATRIX_SUMMARY_KEYS[1], "") == "1"
        and summary_kv.get(CORE_LANG_PROFILE_MATRIX_SUMMARY_KEYS[2], "") == expected_values
        ...
        and summary_kv.get("ci_profile_matrix_gate_selftest_seamgrim_aggregate_summary_status", "") == "pass"
        and summary_kv.get("ci_profile_matrix_gate_selftest_seamgrim_aggregate_summary_ok", "") == "1"
        and summary_kv.get("ci_profile_matrix_gate_selftest_seamgrim_aggregate_summary_values", "")
...
def build_core_lang_sanity_elapsed_markers(sanity_doc: dict[str, object] | None) -> list[str]:
    ...
    total_elapsed_ms = str(sanity_doc.get("total_elapsed_ms", "-")).strip() or "-"
    slowest_step = "-"
    slowest_elapsed_ms = "-"
    steps = sanity_doc.get("steps")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run CI profile gate for core_lang")
    parser.add_argument(
        "--quick",
        action="store_true",
    ...
    temp_ctx = (
        tempfile.TemporaryDirectory(prefix="ci_profile_core_lang_gate_")
    ...
    with temp_ctx as td:
        ...
        if any(marker not in stdout for marker in sanity_required_markers):
            ...
            return 1
        sanity_doc = load_json(sanity_report)
        for marker in build_core_lang_sanity_elapsed_markers(sanity_doc):
        ...
        print("[ci-profile-core-lang] runtime5 summary is emitted but not required for core_lang contract")

        if use_profile_matrix_full_real_smoke:
            print("[ci-profile-core-lang] profile-matrix full-real smoke enabled")
            if profile_matrix_full_real_smoke_step_timeout_sec > 0.0:
                print(
                    "[ci-profile-core-lang] profile-matrix full-real smoke step timeout sec="
        ...
        if use_profile_matrix_full_real_smoke and (args.quick or truthy_env("DDN_CI_PROFILE_GATE_SKIP_AGGREGATE")):
            print("ci_profile_core_lang_status=fail reason=profile_matrix_full_real_smoke_requires_aggregate")
            return 1
        if use_profile_matrix_full_real_smoke and not use_full_aggregate:
            print("ci_profile_core_lang_status=fail reason=profile_matrix_full_real_smoke_requires_full_aggregate")
            return 1

        if args.quick or truthy_env("DDN_CI_PROFILE_GATE_SKIP_AGGREGATE"):
            if args.quick:
                print("[ci-profile-core-lang] aggregate gate skipped by --quick")
            else:
                print("[ci-profile-core-lang] aggregate gate skipped by DDN_CI_PROFILE_GATE_SKIP_AGGREGATE=1")
            print("ci_profile_core_lang_status=pass")
            return 0

        if use_full_aggregate:
            print("[ci-profile-core-lang] aggregate gate uses full mode")
        else:
            print("[ci-profile-core-lang] aggregate gate uses contract-only mode")
        ...
        aggregate_cmd = [
            ...
            "core_lang,full,seamgrim" if use_full_aggregate else profile,
        ]
        if use_full_aggregate:
        ...
        if aggregate_proc.returncode != 0:
            print("ci_profile_core_lang_status=fail reason=aggregate_gate_failed")
            return aggregate_proc.returncode

        summary_report = report_dir / f"{prefix}.ci_gate_summary.txt"
        if not use_full_aggregate:
            if not summary_report.exists():
                print("ci_profile_core_lang_status=fail reason=aggregate_summary_missing")
                return 1
            summary_kv = parse_summary(summary_report)
            if not age4_proof_summary_ok(summary_kv):
                print("ci_profile_core_lang_status=fail reason=aggregate_summary_age4_proof_mismatch")
                return 1
            ...
            if not runtime_helper_summary_ok(summary_kv):
                print("ci_profile_core_lang_status=fail reason=aggregate_summary_runtime_helper_contract_mismatch")
                return 1
            maybe_force_group_id_summary_mismatch(summary_kv)
            if not group_id_summary_ok(summary_kv):
                print("ci_profile_core_lang_status=fail reason=aggregate_summary_group_id_summary_mismatch")
                return 1
            print("[ci-profile-core-lang] aggregate index check skipped in contract-only mode")
            print("ci_profile_core_lang_status=pass")
            return 0
        ...
        if index_proc.returncode != 0:
            print("ci_profile_core_lang_status=fail reason=aggregate_index_contract_failed")
            return index_proc.returncode

        index_doc = load_json(index_report)
        if not isinstance(index_doc, dict):
            print("ci_profile_core_lang_status=fail reason=aggregate_index_json_invalid")
            return 1
        reports = index_doc.get("reports")
        if not isinstance(reports, dict):
            print("ci_profile_core_lang_status=fail reason=aggregate_index_reports_missing_or_invalid")
            return 1
        fixed64_threeway_inputs_raw = str(reports.get("fixed64_threeway_inputs", "")).strip()
        if not fixed64_threeway_inputs_raw:
            print("ci_profile_core_lang_status=fail reason=aggregate_fixed64_threeway_inputs_path_missing")
            return 1
        fixed64_threeway_inputs_path = Path(fixed64_threeway_inputs_raw.replace("\\", "/"))
        if not fixed64_threeway_inputs_path.exists():
            print("ci_profile_core_lang_status=fail reason=aggregate_fixed64_threeway_inputs_report_missing")
            return 1
        fixed64_threeway_inputs_doc = load_json(fixed64_threeway_inputs_path)
        if not isinstance(fixed64_threeway_inputs_doc, dict):
            print("ci_profile_core_lang_status=fail reason=aggregate_fixed64_threeway_inputs_report_invalid")
            return 1
        if str(fixed64_threeway_inputs_doc.get("schema", "")).strip() != "ddn.fixed64.threeway_inputs.v1":
            print("ci_profile_core_lang_status=fail reason=aggregate_fixed64_threeway_inputs_schema_mismatch")
            return 1
        ...
        if summary_proc.returncode != 0:
            print("ci_profile_core_lang_status=fail reason=aggregate_summary_contract_failed")
        ...
        if summary_kv.get("ci_sanity_gate_profile", "") != profile:
            print("ci_profile_core_lang_status=fail reason=aggregate_summary_sanity_profile_mismatch")
            return 1
        if summary_kv.get("ci_sync_readiness_sanity_profile", "") != profile:
            print("ci_profile_core_lang_status=fail reason=aggregate_summary_sync_profile_mismatch")
            return 1
        if summary_kv.get("ci_sanity_pack_golden_lang_consistency_ok", "") != "1":
            print("ci_profile_core_lang_status=fail reason=aggregate_summary_lang_consistency_mismatch")
            return 1
        if summary_kv.get("ci_sanity_pack_golden_metadata_ok", "") != "1":
            print("ci_profile_core_lang_status=fail reason=aggregate_summary_pack_golden_metadata_mismatch")
            return 1
        if summary_kv.get("ci_sanity_pack_golden_graph_export_ok", "") != "1":
            print("ci_profile_core_lang_status=fail reason=aggregate_summary_pack_golden_graph_export_mismatch")
            return 1
        if summary_kv.get("ci_sync_readiness_ci_sanity_pack_golden_graph_export_ok", "") != "1":
            print("ci_profile_core_lang_status=fail reason=aggregate_summary_sync_pack_golden_graph_export_mismatch")
            return 1
        if summary_kv.get("ci_sanity_seamgrim_numeric_factor_policy_ok", "") != "na":
            print("ci_profile_core_lang_status=fail reason=aggregate_summary_sanity_numeric_factor_policy_mismatch")
            return 1
        if summary_kv.get("ci_sync_readiness_ci_sanity_seamgrim_numeric_factor_policy_ok", "") != "na":
            print("ci_profile_core_lang_status=fail reason=aggregate_summary_sync_numeric_factor_policy_mismatch")
            return 1
        if summary_kv.get("ci_sanity_canon_ast_dpack_ok", "") != "1":
            print("ci_profile_core_lang_status=fail reason=aggregate_summary_canon_ast_dpack_mismatch")
            return 1
        if not age4_proof_summary_ok(summary_kv):
            print("ci_profile_core_lang_status=fail reason=aggregate_summary_age4_proof_mismatch")
            return 1
        ...
        if not group_id_summary_ok(summary_kv):
            print("ci_profile_core_lang_status=fail reason=aggregate_summary_group_id_summary_mismatch")
            return 1
        if not profile_matrix_summary_ok(summary_kv, profile, CORE_LANG_PROFILE_MATRIX_VALUES):
            print("ci_profile_core_lang_status=fail reason=aggregate_summary_profile_matrix_contract_mismatch")
            return 1
        print("[ci-profile-core-lang] aggregate summary sanity markers ok")
        ...
        if emit_artifacts_proc.returncode != 0:
            print("ci_profile_core_lang_status=fail reason=aggregate_emit_artifacts_contract_failed")
        ...
        if use_profile_matrix_full_real_smoke:
            ...
            if smoke_proc.returncode != 0:
                print("ci_profile_core_lang_status=fail reason=profile_matrix_full_real_smoke_failed")
                return smoke_proc.returncode
            if PROFILE_MATRIX_FULL_REAL_SMOKE_STATUS_PASS not in smoke_stdout:
                print("ci_profile_core_lang_status=fail reason=profile_matrix_full_real_smoke_marker_missing")
                return 1
