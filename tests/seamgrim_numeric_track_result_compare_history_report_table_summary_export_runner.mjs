...
const OK = "seamgrim_numeric_track_result_compare_history_report_table_summary_export: ok";
...
async function main() {
  ...
  try {
    ...
    await page.waitForFunction(() => window.__SEAMGRIM_NUMERIC_TRACK_RESULT_COMPARE_HISTORY_REPORT_TABLE_SUMMARY__?.schema === "seamgrim.numeric_track_result_compare_history_report_table_summary.v1");
    await page.waitForSelector(".numeric-track-compare-history-report-table tbody tr");
    await page.waitForSelector("#btn-copy-numeric-track-result-compare-history-report-table-summary-export");
    await page.click("#btn-copy-numeric-track-result-compare-history-report-table-summary-export");
    await page.waitForFunction(() => window.__SEAMGRIM_NUMERIC_TRACK_RESULT_COMPARE_HISTORY_REPORT_TABLE_SUMMARY_EXPORT__?.schema === "seamgrim.numeric_track_result_compare_history_report_table_summary_export.v1");
    ...
    const result = await page.evaluate(() => ({
      ...
      summary: window.__SEAMGRIM_NUMERIC_TRACK_RESULT_COMPARE_HISTORY_REPORT_TABLE_SUMMARY__,
      summaryText: window.__SEAMGRIM_NUMERIC_TRACK_RESULT_COMPARE_HISTORY_REPORT_TABLE_SUMMARY_TEXT__,
      summaryExport: window.__SEAMGRIM_NUMERIC_TRACK_RESULT_COMPARE_HISTORY_REPORT_TABLE_SUMMARY_EXPORT__,
      exportText: window.__SEAMGRIM_NUMERIC_TRACK_RESULT_COMPARE_HISTORY_REPORT_TABLE_SUMMARY_EXPORT_TEXT__,
      ...
      summaryTextContent: document.querySelector(".numeric-track-compare-history-report-table-summary")?.textContent ?? "",
    ...
    assert(result.summary.track_id === TRACK_ID, `summary track mismatch: ${result.summary.track_id}`);
    assert(result.summary.source_schema === "seamgrim.numeric_track_result_compare_history_report_table.v1", "summary source schema mismatch");
    assert(result.summary.summary_claim === "metadata_summary", `summary claim mismatch: ${result.summary.summary_claim}`);
    assert(result.summary.replay_claim === false, "summary must not claim replay");
    assert(result.summary.pair_count === 2, `summary pair count mismatch: ${result.summary.pair_count}`);
    assert(result.summary.column_count === 8, `summary column count mismatch: ${result.summary.column_count}`);
    assert(result.summary.lesson_count === 3, `summary lesson count mismatch: ${result.summary.lesson_count}`);
    assert(result.summary.lesson_ids?.join("|") === `${ECON_ID}|${PHYS_ID}|${MATH_ID}`, "summary lesson order mismatch");
    assert(result.summary.state_hash_changed_count === 2, `summary hash changed count mismatch: ${result.summary.state_hash_changed_count}`);
    assert(result.summary.state_hash_same_count === 0, `summary hash same count mismatch: ${result.summary.state_hash_same_count}`);
    assert(result.summary.same_run_kind_count === 1, `summary same run-kind count mismatch: ${result.summary.same_run_kind_count}`);
    assert(result.summary.run_kind_changed_count === 1, `summary run-kind changed count mismatch: ${result.summary.run_kind_changed_count}`);
    assert(result.summary.channel_delta_total === 1, `summary channel total mismatch: ${result.summary.channel_delta_total}`);
    assert(result.summary.channel_delta_abs_total === 1, `summary channel abs total mismatch: ${result.summary.channel_delta_abs_total}`);
    assert(String(result.summaryText).includes("schema\tseamgrim.numeric_track_result_compare_history_report_table_summary.v1"), "summary text schema missing");
    assert(String(result.summaryText).includes("summary_claim\tmetadata_summary"), "summary text claim missing");
    assert(String(result.summaryText).includes(`lesson_ids\t${ECON_ID}|${PHYS_ID}|${MATH_ID}`), "summary text lesson ids missing");
    assert(String(result.summaryTextContent).includes("교과 3"), "panel lesson summary missing");
    assert(String(result.summaryTextContent).includes("hash 변화 2"), "panel hash summary missing");
    assert(String(result.summaryTextContent).includes("실행종류 변화 1"), "panel run-kind summary missing");
    assert(String(result.summaryTextContent).includes("채널 변화절대합 1"), "panel channel summary missing");
    assert(String(result.summaryTextContent).includes("요약 복사"), "summary export copy action missing");
    assert(result.summaryExport.track_id === TRACK_ID, `export track mismatch: ${result.summaryExport.track_id}`);
    assert(result.summaryExport.source_schema === "seamgrim.numeric_track_result_compare_history_report_table_summary.v1", "export source schema mismatch");
    assert(result.summaryExport.export_claim === "metadata_text", `export claim mismatch: ${result.summaryExport.export_claim}`);
    assert(result.summaryExport.replay_claim === false, "summary export must not claim replay");
    assert(result.summaryExport.pair_count === 2, `export pair count mismatch: ${result.summaryExport.pair_count}`);
    assert(result.summaryExport.column_count === 8, `export column count mismatch: ${result.summaryExport.column_count}`);
    assert(result.summaryExport.lesson_count === 3, `export lesson count mismatch: ${result.summaryExport.lesson_count}`);
    assert(result.summaryExport.state_hash_changed_count === 2, `export hash count mismatch: ${result.summaryExport.state_hash_changed_count}`);
    assert(result.summaryExport.run_kind_changed_count === 1, `export run kind count mismatch: ${result.summaryExport.run_kind_changed_count}`);
    assert(result.summaryExport.channel_delta_total === 1, `export channel total mismatch: ${result.summaryExport.channel_delta_total}`);
    assert(result.summaryExport.channel_delta_abs_total === 1, `export channel abs total mismatch: ${result.summaryExport.channel_delta_abs_total}`);
    assert(result.summaryExport.summary?.schema === "seamgrim.numeric_track_result_compare_history_report_table_summary.v1", "embedded summary missing");
    assert(String(result.summaryExport.summary_text).includes("summary_claim\tmetadata_summary"), "embedded summary text missing");
    assert(String(result.exportText).includes("schema\tseamgrim.numeric_track_result_compare_history_report_table_summary_export.v1"), "export text schema missing");
    assert(String(result.exportText).includes("source_schema\tseamgrim.numeric_track_result_compare_history_report_table_summary.v1"), "export text source missing");
    ...
    assert(String(result.exportText).includes("summary_text"), "export text summary section missing");
