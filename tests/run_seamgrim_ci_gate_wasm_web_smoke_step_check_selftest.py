...
CHECK_SCRIPT = Path(__file__).resolve().parent / "run_seamgrim_ci_gate_wasm_web_smoke_step_check.py"
CHECK_REPORT_SCHEMA = "ddn.seamgrim_ci_gate_wasm_web_smoke_step_check.v1"
EXPECTED_FILE_COUNT_FLOOR = 20
MISSING_TOKEN_REL_PATH = "tests/run_age3_completion_gate.py"
MISSING_TOKEN_VALUE = '"seamgrim_wasm_web_step_check_pass"'
MISSING_FILE_PREFERRED = "tests/run_ci_gate_report_index_check.py"


def fail(msg: str) -> int:
    print(f"[seamgrim-ci-gate-wasm-web-smoke-step-check-selftest] fail {msg}", file=sys.stderr)
    return 1
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Selftest for seamgrim wasm/web smoke step contract checker"
    )
    parser.add_argument(
        "--verify-report",
        help="optional real report path emitted by run_seamgrim_ci_gate_wasm_web_smoke_step_check.py",
    )
    return parser.parse_args()


def load_check_module() -> ModuleType:
    spec = importlib.util.spec_from_file_location("seamgrim_wasm_web_step_check_mod", CHECK_SCRIPT)
...
def validate_real_report(path: Path, minimum_checked_files: int) -> str | None:
    ...
    if checked_files < minimum_checked_files:
        return (
            "verify-report checked_files too small "
            f"checked_files={checked_files} minimum={minimum_checked_files}"
        )
    try:
...
def main() -> int:
    args = parse_args()
    with tempfile.TemporaryDirectory(prefix="seamgrim_wasm_web_smoke_step_selftest_") as tmp:
        root = Path(tmp)
        requirements = load_requirements()
        if len(requirements) < EXPECTED_FILE_COUNT_FLOOR:
            return fail(
                f"requirement count too small count={len(requirements)} floor={EXPECTED_FILE_COUNT_FLOOR}"
            )
        ...
        write_text(
            root / "tests" / "run_ci_dummy_untracked_contract.py",
            '\n'.join(
                [
        ...
        untracked_proc = run_check(root)
        if untracked_proc.returncode == 0:
            return fail("untracked contract file case must fail")
        if "run_ci_dummy_untracked_contract.py::untracked_contract_file" not in untracked_proc.stdout:
            return fail(f"untracked contract marker not found out={untracked_proc.stdout} err={untracked_proc.stderr}")
        (root / "tests" / "run_ci_dummy_untracked_contract.py").unlink()

        write_text(
            root / "tests" / "run_ci_dummy_untracked_wasm_contract.py",
            '\n'.join(
                [
        ...
        untracked_wasm_proc = run_check(root)
        if untracked_wasm_proc.returncode == 0:
            return fail("untracked wasm contract file case must fail")
        if "run_ci_dummy_untracked_wasm_contract.py::untracked_contract_file" not in untracked_wasm_proc.stdout:
            return fail(
                f"untracked wasm contract marker not found out={untracked_wasm_proc.stdout} err={untracked_wasm_proc.stderr}"
            )
        (root / "tests" / "run_ci_dummy_untracked_wasm_contract.py").unlink()

        missing_file_rel = MISSING_FILE_PREFERRED if MISSING_FILE_PREFERRED in requirements else next(iter(sorted(requirements)))
        (root / missing_file_rel).unlink()
        miss_file_proc = run_check(root)
        if miss_file_proc.returncode == 0:
            return fail("missing file case must fail")
        if f"{missing_file_rel}::missing_file" not in miss_file_proc.stdout:
            return fail(f"missing file marker not found out={miss_file_proc.stdout} err={miss_file_proc.stderr}")

    if args.verify_report:
        verify_error = validate_real_report(
            Path(args.verify_report),
            minimum_checked_files=EXPECTED_FILE_COUNT_FLOOR,
        )
        if verify_error is not None:
            return fail(verify_error)

    print("[seamgrim-ci-gate-wasm-web-smoke-step-check-selftest] ok")
    return 0
