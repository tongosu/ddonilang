#!/usr/bin/env python3
"""Validate ROADMAP_V2_POST_SA1_FRONTIER_REBASE_V1."""
...
REPORT = ROOT / "docs" / "status" / "roadmap_v2" / "POST_SA1_FRONTIER_REBASE_REPORT_20260609.md"
...
PACK = ROOT / "pack" / "roadmap_v2_post_sa1_frontier_rebase_v1"
CONTRACT = PACK / "contract.detjson"
NEXT_FRONTIER = PACK / "next_frontier.detjson"


def fail(message: str) -> None:
    print(f"[roadmap-v2-post-sa1-frontier-rebase] FAIL: {message}", file=sys.stderr)
    raise SystemExit(1)
...
def check_files() -> None:
    for path in [
        ...
        ROOT / "tests" / "run_roadmap_v2_sa1_bogae_graph_space2d_matrix_reconciliation_check.py",
    ]:
        require_file(path)
...
def check_docs() -> None:
    shared = [
        ...
        "ROADMAP_V2 post-SA1 frontier rebase 4/4 = 100%",
        ...
        "pa1_matrix_reconciliation_claim:false",
        "docs_ssot_change:false",
    ]
    for path in [REPORT, DEV_SUMMARY]:
        require_tokens(path, shared)
    require_tokens(PROJECT_STATUS, ["ROADMAP_V2_POST_SA1_FRONTIER_REBASE_V1", "54/90 = 60%", "59/90 = 66%", "파-1"])
    require_tokens(CHANGELOG, ["ROADMAP_V2 post-SA1 frontier rebase", "ROADMAP_V2_PA1_BASELINE_MARKET_FIRST_RUN_MATRIX_RECONCILIATION_V1"])


def check_payloads() -> None:
    expected = {
        "schema": "ddn.pack.contract.v1",
        "pack": "roadmap_v2_post_sa1_frontier_rebase_v1",
        "kind": "roadmap_v2_post_sa1_frontier_rebase",
        "runtime_claim": False,
        "product_code_change": False,
        "product_ui_change": False,
        "econ_runtime_change": False,
        "econ_policy_advice_claim": False,
        "real_world_prediction_claim": False,
        "closed_by": "ROADMAP_V2_POST_SA1_FRONTIER_REBASE_V1",
        "matrix_closure_claim": False,
        "matrix_closure_tier": "닫힘-문서",
        "roadmap_matrix_increment": False,
        "pa1_matrix_reconciliation_claim": False,
        "current_stage": "ROADMAP_V2 post-SA1 frontier rebase",
        ...
        "selected_next_work": "ROADMAP_V2_PA1_BASELINE_MARKET_FIRST_RUN_MATRIX_RECONCILIATION_V1",
        "selected_coordinate": "파-1",
        "rejected_work": "NEW_ECON_RUNTIME_IMPLEMENTATION_WITHOUT_PA1_RECONCILIATION",
        "docs_ssot_change": False,
    ...
    if frontier.get("schema") != "ddn.roadmap_v2.post_sa1_frontier_rebase.v1":
    ...
    for key in [
        ...
        "product_ui_change",
        "econ_runtime_change",
        "econ_policy_advice_claim",
        "real_world_prediction_claim",
        "matrix_closure_claim",
        "roadmap_matrix_increment",
        "pa1_matrix_reconciliation_claim",
        "docs_ssot_change",
    ]:
...
def check_frontier_basis() -> None:
    require_tokens(
        ...
        [
            ...
            "| 2마루 닫힘마루 | bridge/social pack | model-first/agent-first/bridge | bridge report pack | 닫힘-동작 |",
            "| 4 | 파-1 | 기준 시장/세금/가격상한 3개 첫실행 | econ_*_AGE4_READY packs | 닫힘-동작 |",
        ],
    )
    require_tokens(TRACKER, ["| 11 | `파-1` | 기준 시장/세금/가격상한 3개 첫실행 | 닫힘-동작 |", "| 40 | `파-2` | bridge/social pack | 닫힘-동작 |"])
    require_tokens(MANIFEST, ["| `파-1` | `econ_age4_ready_baseline_tax_ceiling_v1`; `roadmap_v2_pa1_baseline_market_first_run_matrix_reconciliation_v1` |"])
...
def check_gates() -> None:
    run([sys.executable, "tests/run_pack_golden.py", "roadmap_v2_post_sa1_frontier_rebase_v1"], timeout=120)
    ...
    run([sys.executable, "tests/run_roadmap_v2_sa1_bogae_graph_space2d_matrix_reconciliation_check.py"], timeout=480)
...
def main() -> None:
    ...
    check_gates()
    print("[roadmap-v2-post-sa1-frontier-rebase] OK")
