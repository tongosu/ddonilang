#!/usr/bin/env python
"""DDN 실행 결과에서 seamgrim.structure.v0 JSON을 추출한다."""
import argparse
import json
import sys
import tempfile
...
from bridge_server import parse_structure_blocks


def build_structure_doc(structure_data: dict, source_hash: str, meta: dict) -> dict:
    return {
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Export seamgrim.structure.v0 from DDN output")
    ...
    if args.output:
        ...
        if args.auto_name:
            ...
            name = f"{stem}_{stamp}_structure.json"
        else:
            name = f"{stem}_structure.json"
    ...
    if not structure_data:
        print("경고: structure 데이터가 없습니다.", file=sys.stderr)
        return 1

    structure_doc = build_structure_doc(structure_data, source_hash, meta)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(
        json.dumps(structure_doc, ensure_ascii=False, indent=2, sort_keys=True),
        encoding="utf-8",
    )
