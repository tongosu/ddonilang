...
> 공개용 역사 요약입니다. 현재 셈그림 작업실/current-line 실행과 예제 상태는 `README.md`, `QUICKSTART.md`, `DDONIRANG_DEV_STRUCTURE.md`를 함께 확인합니다.
...
- 파일 메타:
`#키: 값` 형태의 메타 헤더는 과거 호환 입력으로 남을 수 있다. 현재 셈그림 예제 rail은 `설정 { 제목: ... 설명: ... }.` 형식을 우선한다.
- 주석:
`// ...` 한 줄 주석, `/* ... */` 블록 주석(중첩 금지).
- 기호의 의미는 고정:
...
- 이 문서는 과거 Gate0 정본 요약을 포함한다.
- 현재 공개 README/셈그림 current-line 예제는 루트 접두 없이 `채비`, top-level 값, `보여주기`, `보임`, `모양`을 우선한다.
- 과거 루트 접두/숨김 모드 설명은 호환 배경으로만 읽고, 공개 첫 예제에는 쓰지 않는다.
...
- 접미 체인 순서:
`값 [@단위|@"자원"] [:핀] [~조사]`
- `~`가 없으면 조사 자동 분리 규칙을 적용하며, 전체 이름/조사 분리가 모두 성립하면 오류다.
- 모호성은 `:핀` 또는 `~조사`로 해소해야 한다.
- 입력에서 `,`는 읽기 쉼표로 허용될 수 있으나 정본 출력에서는 제거된다.
...
고르기:
  ...
  아니면:
    "0 이하" 보여주기.
```
- 조건 분기는 `{ ... }인것:` 형태만 허용하며, `아니면:`은 반드시 마지막이어야 한다.
- 조건 블록에서는 `<-`, `}하고`, I/O, 운(Random) 소비 같은 부작용을 금지한다.
- `~해보고:`는 시도/바인딩. 결과는 `그것`에 바인딩된다.
- 반복:
`반복:`, `~동안:`, `~에 대해:`.
- 머릿말은 **닫힌 집합**이다. 임의의 머릿말 추가는 금지.
- `(임자)마다`, `(임자)모두`에서 `(임자)`는 블록 지역 변수다.
- `모두 {}`는 `(임자) 대상차림에 대해: { ... }`의 설탕이다.
...
- 계약 모드는 `바탕으로(알림|중단)`, `다짐하고(알림|중단)`으로 지정하며 기본은 `중단`이다.
- 전제 실패는 해당 계약 구역을 **거부(Reject)** 하고 진행하지 않는다.
- 계약 조건은 순수해야 하며 운(Random) 사용은 금지된다.
- 검사 훅:
...
- 기본 알: 수, 정수 계열, 참거짓, 글, 원자(`#...`).
- 복합: 이름씨(구조체), 묶음씨(레코드), 튜플, 고름씨(태그 유니온), 값꾸러미.
- `T?`는 옵션 타입이며 값 `없음`을 가진다.
- `나`는 자기 참조를 위한 예약 키워드다.
...
- 튜플은 정규화 규칙을 가지며, 고름씨 생성은 `#태그(...)` 형태를 쓴다.
- 고름씨 태그는 전역 유일성 규칙을 가진다.
...
- 표현식 우선순위는 SSOT에 고정되어 있으며, 의미를 바꾸는 괄호만 유지된다.
- 확장 이음기호는 정해진 집합에서 1~3글자 조합을 허용한다.
...
- `@"경로"`는 `"경로" 쓸감.`의 문법 설탕이다.
- `@식별자`는 쓸감 리터럴이 아니며, `@`는 단위/쓸감 접미로만 사용한다.
- `자산`은 치명 금지어이며 `자원`은 바탕의 레거시 별칭이다.
...
- 말결(퍼지) 토큰은 `$` 정본을 사용한다.
예: `$매우`, `$조금`.
- 실행 어미: `-고`는 순차, `-며`는 병행, `-다.`는 종결이다.
- 확장 어미(`-거나/-다가/-려다/-채/-자마자/-도록`)는 점진 도입 항목이다.
- 어간 교대는 `~` 별칭으로 선언하며, 결정적 파이프라인으로 처리한다.
...
- 표준 훅: `(시작)할때`, `(끝)할때`, `(매마디)마다`, `(#태그)할때`.
- 알림 디스패치는 태그/등록 순서 기준으로 결정적 정렬을 유지한다.
...
- 군집 제어는 `모두 {}` 설탕을 포함한다.
- 쿼리 문법: `집합 = (조건1, 조건2, ...) 임자들.`
- 필터: `이름씨`, `#알`, `!이름씨`, `!#알`, `(필드 > 값)` 같은 순수 술어.
- 결과 정렬은 `(realm_id, entity_id)` 오름차순이 정본이다.
- 군집 블록은 `모두`, `첫째/마지막`, `개수` 패턴을 포함한다.
...
- `묶음.필드`/`짝맞춤.키` 같은 점접근과 차림 연산의 정본화가 연결된다.
...
참고:
`보상 (값:수 ~만큼)`처럼 “이름+인자” 형태로 적힌 것은 **PinSpec 설명 표기(정본문법 아님)**이며,
실제 문법 호출은 `(값) 보상` 형태를 따른다.
